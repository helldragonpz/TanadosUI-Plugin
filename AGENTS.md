...
Use `dotnet build JMSFusion.csproj` for a fast compile check. Use `dotnet publish JMSFusion.csproj -c Release` to produce the plugin assembly with embedded assets. Run `./scripts/package-tanados-ui.sh 2.9.0.1` to build the installable server zip in `dist/` and refresh `manifest.json` with the MD5 checksum. Run `bash ./update_meta.sh 2.9.0.1` only when you need to refresh `meta.json` timestamps or versioned icon paths without packaging.
