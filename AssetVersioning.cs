...
(function () {
  ...
  var PING_PATHS = ["/TanadosUI/ping", "/Plugins/TanadosUI/ping"];
  ...
  function detectBrowserLangKey() {
    ...
    for (var i = 0; i < candidates.length; i += 1) {
      ...
      if (value === "bul" || value === "bul_bg" || base === "bg" || base === "bul") return "bul";
    }
  ...
  function resolveLangKey(raw) {
    ...
    if (value === "tur" || value === "eng" || value === "deu" || value === "fre" || value === "spa" || value === "rus" || value === "bul") {
      return value;
    }
    if (value === "fra") return "fre";
    if (value === "bul_bg") return "bul";
    var base = value.split(/[-_]/)[0];
    ...
    if (base === "bg" || base === "bul") return "bul";
  ...
  var splashLocale = {
    ...
    bul: {
      stageLock: "ЗАКЛЮЧВАНЕ",
      detailLock: "Слоят на обвивката се фиксира",
      stageStructure: "ЯДРО",
      detailStructure: "Ядрото на интерфейса се синхронизира",
      stageTakeover: "ПОЕМАНЕ",
      detailTakeover: "Двигателят на {title} поема управлението",
      stageFlow: "ПОТОК",
      detailFlow: "Метриките за зареждане в реално време се синхронизират",
      stageFallback: "РЕЗЕРВ",
      detailFallback: "Връщане към интерфейса на Jellyfin",
      stageReady: "ГОТОВО",
      detailReady: "{title} е онлайн"
    }
  };
...
""";
        ...
        {
            var assembly = typeof(TanadosUIPlugin).Assembly;
