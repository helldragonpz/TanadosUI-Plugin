...
{
    [ApiController]
    [Route("Plugins/TanadosUI")]
    public class TanadosUIApiController : ControllerBase
    {
        private readonly ILogger<TanadosUIApiController> _logger;
        public TanadosUIApiController(ILogger<TanadosUIApiController> logger) => _logger = logger;

        [HttpGet("Configuration")]
        public ActionResult<TanadosUIConfiguration> GetConfiguration() => Ok(TanadosUIPlugin.Instance.Configuration);
        ...
        public IActionResult UpdateConfiguration([FromBody] UpdateRequest req)
        {
            try
            {
                ...
                _logger.LogInformation("[TanadosUI] CFG SAVED: dir='{dir}', player='{sub}'", cfg.ScriptDirectory, cfg.PlayerSubdir);
        ...
        public ContentResult GetSnippet()
        {
            ...
            var html = TanadosUIPlugin.Instance.BuildScriptsHtml(pathBase);
        ...
        public IActionResult Patch()
        {
            try
            {
                ...
                var pathBase = HttpContext?.Request.PathBase.Value ?? string.Empty;
                var snippet = TanadosUIPlugin.Instance.BuildScriptsHtml(pathBase);
                ...
                _logger.LogInformation("[TanadosUI] Patched index.html at {Index}", indexPath);
        ...
        public IActionResult Unpatch()
        {
            try
            {
                ...
                _logger.LogInformation("[TanadosUI] Unpatched index.html at {Index}", indexPath);
                return Ok(new { success = true, path = indexPath });
            }
            catch (UnauthorizedAccessException ex)
            {
