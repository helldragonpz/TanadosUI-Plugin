...
public sealed class TrailerAutomationService
{
    ...
    private const string DefaultToolDirName = "TanadosUI-tools";
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                _logger.LogInformation(
                    "[TanadosUI] Tool bootstrap hazır. yt-dlp={YtDlpVersion} deno={DenoVersion} root={ToolRoot}",
            ...
            {
                _logger.LogWarning(ex, "[TanadosUI] Tool bootstrap başarısız.");
            }
        });
    }
...
";
    ...
    private string ResolveManagedToolRoot()
    {
        var candidates = new[]
        {
            CombinePath(_applicationPaths.DataPath, "TanadosUI", DefaultToolDirName),
            CombinePath(_applicationPaths.ProgramDataPath, "TanadosUI", DefaultToolDirName),
            CombinePath(_applicationPaths.PluginsPath, "TanadosUI", DefaultToolDirName),
            CombinePath(_applicationPaths.CachePath, "TanadosUI", DefaultToolDirName),
    ...
    private async Task<ManagedToolSuite> EnsureManagedToolSuiteAsync(CancellationToken ct)
    {
        ...
        try
        {
                ...
                string.IsNullOrWhiteSpace(ResolveBundledFfToolPath("ffprobe")))
            {
                ...
                {
                    _logger.LogWarning(ex, "[TanadosUI] ffmpeg/ffprobe yönetilen indirme başarısız.");
                }
            }
    ...
    private async Task<GitHubReleaseResponse?> TryGetLatestGitHubReleaseAsync(string apiUrl, CancellationToken ct)
    {
        try
        {
            ...
            req.Headers.TryAddWithoutValidation("User-Agent", "TanadosUI/2.0");
            req.Headers.TryAddWithoutValidation("Accept", "application/vnd.github+json");

            using var resp = await Http.SendAsync(req, ct).ConfigureAwait(false);
            if (!resp.IsSuccessStatusCode)
            {
                _logger.LogWarning("[TanadosUI] Release sorgusu başarısız: {ApiUrl} status={StatusCode}", apiUrl, (int)resp.StatusCode);
                return null;
            }
        ...
        {
            _logger.LogWarning(ex, "[TanadosUI] Release bilgisi alınamadı: {ApiUrl}", apiUrl);
            return null;
        }
    }
