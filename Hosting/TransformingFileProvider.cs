...
namespace Jellyfin.Plugin.TanadosUI
{
    public sealed class TransformingFileProvider : IFileProvider
    {
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            try
            {
                _logger.LogDebug("[TanadosUI][DIAG] TransformingFileProvider hit: {Subpath}", subpath);
                ...
                {
                    _logger.LogDebug("[TanadosUI][DIAG] Already contains snippet, returning original.");
                    return info;
                }
                ...
                {
                    _logger.LogWarning("[TanadosUI][DIAG] BuildScriptsHtml returned empty snippet; returning original.");
                    return info;
                }
                ...
                _logger.LogInformation("[TanadosUI][DIAG] In-memory rewritten: {Path} (bytes={Len})", subpath, payload.Length);
                return new RewritingFileInfo(info, payload);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "[TanadosUI][DIAG] In-memory rewrite failed for {Path}; serving original.", subpath);
                return info;
            }
        }
