...
namespace Jellyfin.Plugin.TanadosUI
{
    public sealed class InMemoryRewriterFileProvider : IFileProvider
    {
        ...
        public void SetUnderlying(IFileProvider provider, ILogger logger)
        {
            ...
            _logger.LogInformation("[TanadosUI] Registered in-memory transformer over static files");
        }
        ...
        public IFileInfo GetFileInfo(string subpath)
        {
            ...
            {
                _diagLogged++;
                _logger.LogInformation("[TanadosUI][DIAG] GetFileInfo subpath='{Subpath}'", subpath);
            }
            ...
            if (!original.Exists)
            {
                ...
                {
                    _diagLogged++;
                    _logger.LogInformation("[TanadosUI][DIAG] original file NOT FOUND for '{Subpath}'", subpath);
                }
                return original;
            }
            ...
            try
            {
                ...
                {
                    _diagLogged++;
                    _logger.LogInformation("[TanadosUI][DIAG] loaded html ({Len} chars) from '{Subpath}'", html.Length, subpath);
                }
                    ...
                    html.Contains("<!-- SL-INJECT END -->", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        _diagLogged++;
                        _logger.LogInformation("[TanadosUI][DIAG] markers already present, returning original for '{Subpath}'", subpath);
                    }
                    return original;
                }

                var snippet = TanadosUIPlugin.Instance?.BuildScriptsHtml("") ?? "";
                if (string.IsNullOrEmpty(snippet))
                {
                    if (_diagLogged < MaxDiagLogs)
                    {
                        _diagLogged++;
                        _logger.LogWarning("[TanadosUI][DIAG] snippet is empty; returning original for '{Subpath}'", subpath);
                    }
                    return original;
                }
                ...
                _logger.LogInformation("[TanadosUI] In-memory rewritten: {Path} ({Bytes} bytes)", subpath, resultBytes.Length);
                return new RewritingFileInfo(original, resultBytes);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "[TanadosUI] In-memory rewrite failed for {Path}. Falling back to original.", subpath);
                return original;
            }
        }
