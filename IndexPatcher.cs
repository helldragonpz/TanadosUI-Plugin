...
namespace Jellyfin.Plugin.TanadosUI
{
    public static class IndexPatcher
    {
        ...
        private static string GetBackupPath(string path) => path + ".TanadosUI.bak";
        ...
        private static bool IsWritable(string path, ILogger logger)
        {
            ...
            {
                logger.LogWarning(ex, "[TanadosUI] No write permission: {Path}", path);
                return false;
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "[TanadosUI] Write probe failed: {Path}", path);
                return false;
            }
        }
        ...
        private static void EnsureBackup(string path, ILogger logger)
        {
            try
            {
                ...
                if (!File.Exists(backupPath))
                {
                    ...
                    logger.LogInformation("[TanadosUI] Backup created: {BackupPath}", backupPath);
                }
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "[TanadosUI] Backup creation failed for: {Path}", path);
            }
        }
        ...
        private static void DeleteBackupIfPresent(string path, ILogger logger)
        {
            try
            {
                ...
                logger.LogInformation("[TanadosUI] Removed backup: {BackupPath}", backupPath);
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "[TanadosUI] Failed removing backup for: {Path}", path);
            }
        }
        ...
        private static void WriteCompressedCopiesIfPresent(ILogger logger, string webRootPath, string html)
        {
            ...
            try
            {
                if (File.Exists(gz) && IsWritable(gz, logger))
                {
                    ...
                    logger.LogInformation("[TanadosUI] index.html.gz updated");
                }
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "[TanadosUI] Failed updating index.html.gz");
            }
            ...
            try
            {
                if (File.Exists(br) && IsWritable(br, logger))
                {
                    ...
                    logger.LogInformation("[TanadosUI] index.html.br updated");
                }
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "[TanadosUI] Failed updating index.html.br");
            }
        }
        ...
        public static bool EnsurePatched(ILogger logger, string webRootPath, string? pathBase = null)
        {
            try
            {
                logger.LogInformation("[TanadosUI] Checking web root: {WebRoot}", webRootPath);

                var indexPath = Path.Combine(webRootPath, "index.html");
                logger.LogInformation("[TanadosUI] Index path: {IndexPath}", indexPath);

                if (!File.Exists(indexPath))
                {
                    logger.LogWarning("[TanadosUI] index.html not found at: {Path}", indexPath);
                    return false;
                }
                ...
                if (start >= 0 && end >= 0)
                {
                    ...
                    {
                        logger.LogInformation("[TanadosUI] index.html patch is already up to date");
                        return true;
                    }

                    html = html.Remove(start, end - start).Insert(start, block);
                    logger.LogInformation("[TanadosUI] Existing inject block refreshed");
                }
                else
                {
                    ...
                    if (headEndPos >= 0)
                    {
                        ...
                        logger.LogInformation("[TanadosUI] Found </head> tag at position: {Position}", headEndPos);
                    }
                    else
                    {
                        ...
                        if (bodyEndPos >= 0)
                        {
                            ...
                            logger.LogInformation("[TanadosUI] Found </body> tag at position: {Position}", bodyEndPos);
                        }
                        else
                        {
                            html += Environment.NewLine + block + Environment.NewLine;
                            logger.LogWarning("[TanadosUI] Neither </head> nor </body> tag found, appended to end");
                        }
                    }
                }
                ...
                logger.LogInformation("[TanadosUI] index.html updated successfully");
                ...
                {
                    logger.LogInformation("[TanadosUI] Patch verification successful");
                    WriteCompressedCopiesIfPresent(logger, webRootPath, verify);
                    return true;
                }

                logger.LogError("[TanadosUI] Patch verification FAILED");
                return false;
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "[TanadosUI] Failed to patch index.html");
                return false;
            }
        }
        ...
        public static bool EnsureUnpatched(ILogger logger, string webRootPath)
        {
            try
            {
                ...
                if (!File.Exists(indexPath))
                {
                    logger.LogWarning("[TanadosUI] Unpatch: index.html not found: {Path}", indexPath);
                    return false;
                }
                ...
                if (!hasInjectBlock)
                {
                    logger.LogInformation("[TanadosUI] Unpatch: inject block not found (already clean)");
                    if (hasBackup)
                    {
                        if (IsWritable(indexPath, logger))
                        {
                ...
                if (hasBackup)
                {
                    try
                    {
                        ...
                        logger.LogInformation("[TanadosUI] Unpatch: restored from backup: {BackupPath}", backupPath);
                    ...
                    {
                        logger.LogWarning(ex, "[TanadosUI] Unpatch: failed to restore backup, falling back to inline removal");
                    }
                }
                ...
                File.WriteAllText(indexPath, html, Encoding.UTF8);
                logger.LogInformation("[TanadosUI] Unpatch: inject block removed");
                WriteCompressedCopiesIfPresent(logger, webRootPath, html);
                DeleteBackupsIfPresent(logger, webRootPath);
                return true;
            }
            ...
            {
                logger.LogError(ex, "[TanadosUI] EnsureUnpatched failed");
                return false;
            }
        }
    }
