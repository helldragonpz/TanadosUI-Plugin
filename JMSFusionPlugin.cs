...
namespace Jellyfin.Plugin.TanadosUI
{
    public class TanadosUIPlugin : BasePlugin<TanadosUIConfiguration>, IHasWebPages
    {
        ...
        public static TanadosUIPlugin Instance { get; private set; } = null!;

        public TanadosUIPlugin(IApplicationPaths paths, IXmlSerializer xmlSerializer, ILoggerFactory loggerFactory)
            : base(paths, xmlSerializer)
        {
            _logger = loggerFactory.CreateLogger<TanadosUIPlugin>();
            _paths = paths;
            Instance = this;
            ...
            {
                _logger.LogInformation("[TanadosUI] Configuration changed.");
            ...
            try
            {
                if (Configuration.EnableTransformEngine)
                {
                    ResponseTransformation.Register(@".*index\.html(\.gz|\.br)?$",
                        req =>
                        {
                            ...
                            _logger.LogInformation(
                                "[TanadosUI][DIAG] Transform hit for {Path} (len={Len})",
                    ...
                    _logger.LogInformation("[TanadosUI] Registered in-memory transformation rule for .*index.html(+gz/br)");
                }
                else
                {
                    _logger.LogInformation("[TanadosUI] Transform engine disabled by configuration");
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "[TanadosUI] Failed to register in-memory transformation; middleware/patch fallback will be used.");
            }
        }

        public override void OnUninstalling()
        {
            _logger.LogInformation("[TanadosUI] Plugin uninstall detected. Cleaning physical index.html patch if present.");
            TryUnpatchIndexHtml();
            base.OnUninstalling();
        }
        ...
        private string? DetectWebRoot()
        {
            try
            {
                ...
                {
                    _logger.LogInformation("[TanadosUI] Using ApplicationPaths.WebPath as web root: {WebRoot}", webPath);
                    return webPath;
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "[TanadosUI] Failed probing ApplicationPaths.WebPath");
            }
            ...
            foreach (var p in candidates)
            {
                try
                {
                    _logger.LogInformation("[TanadosUI] Checking web root candidate: {Candidate}", p);

                    if (Directory.Exists(p) && File.Exists(Path.Combine(p, "index.html")))
                    {
                        _logger.LogInformation("[TanadosUI] Found web root: {WebRoot}", p);
                        return p;
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "[TanadosUI] Error checking candidate: {Candidate}", p);
                }
            }

            _logger.LogWarning("[TanadosUI] Web root not found in any candidate location");
            return null;
        }
        ...
        public void TryPatchIndexHtml()
        {
            try
            {
                ...
                {
                    _logger.LogWarning("[TanadosUI] Web root not found; skipping patch.");
                    return;
                }

                var ok = IndexPatcher.EnsurePatched(_logger, root);
                _logger.LogInformation("[TanadosUI] Patch result: {ok}", ok);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[TanadosUI] TryPatchIndexHtml failed");
            }
        }
        ...
        public void TryUnpatchIndexHtml()
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(root))
                {
                    _logger.LogWarning("[TanadosUI] Web root not found; skipping unpatch.");
                    return;
                }

                var ok = IndexPatcher.EnsureUnpatched(_logger, root);
                _logger.LogInformation("[TanadosUI] Unpatch result: {ok}", ok);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[TanadosUI] TryUnpatchIndexHtml failed");
            }
        }
        ...
        public string GetStorageDirectory(params string[] segments)
        {
            ...
            var current = Path.Combine(basePath, "TanadosUI");
