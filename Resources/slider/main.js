import { saveCredentials, saveApiKey, getAuthToken } from "../Plugins/TanadosUI/runtime/auth.js";
import {
  getConfig,
...
import { getSessionInfo, getAuthHeader, waitForAuthReadyStrict, isAuthReadyStrict, AUTH_PROFILE_CHANGED_EVENT, USERDATA_CHANGED_EVENT } from "../Plugins/TanadosUI/runtime/api.js";
...
const CUSTOM_SPLASH_PING_PATHS = ["/TanadosUI/ping", "/Plugins/TanadosUI/ping"];
...
function getCustomSplashCandidateSlide(targetSlide = null) {
  ...
  return document.querySelector(
    "#indexPage:not(.hide) #TanadosUI-slides-container .TanadosUI-slide.active, " +
    "#homePage:not(.hide) #TanadosUI-slides-container .TanadosUI-slide.active, " +
    "#TanadosUI-slides-container .TanadosUI-slide.active, " +
    "#indexPage:not(.hide) #TanadosUI-slides-container .TanadosUI-slide, " +
    "#homePage:not(.hide) #TanadosUI-slides-container .TanadosUI-slide, " +
    "#TanadosUI-slides-container .TanadosUI-slide"
  );
}
...
function isCustomSplashSlideVisuallyReady(targetSlide = null) {
  ...
  const container = slide.closest?.("#TanadosUI-slides-container");
...
function finalizeCustomSplashHide(reason = "ready") {
  ...
  const hasSlides = !!document.querySelector(
    "#indexPage:not(.hide) .TanadosUI-slide, #homePage:not(.hide) .TanadosUI-slide"
  );
  if (hasSlides) {
...
function hideCustomSplash(reason = "ready") {
  ...
  const closingDetail = reason === "timeout"
    ...
    : splashLabel("customSplashDetailReady", "TanadosUI çevrimiçi");
...
function isSliderBootTokenCurrent(token, { requireHomeVisible = true, requireContainer = false } = {}) {
  ...
  if (requireContainer) {
    return !!document.querySelector(
      "#indexPage:not(.hide) #TanadosUI-slides-container, #homePage:not(.hide) #TanadosUI-slides-container"
    );
  }
  return true;
}
...
(function earlyCssBoot(){
  ...
  const criticalCSS = `
    ...
    html[data-jms-home-slider-only="1"][data-jms-home-tab-active="0"] #TanadosUI-slides-container,
    html[data-jms-home-slider-only="1"][data-jms-home-tab-active="0"] .TanadosUI-slide-progress-bar,
    html[data-jms-home-slider-only="1"][data-jms-home-tab-active="0"] .TanadosUI-slide-progress-seconds,
    html[data-jms-home-slider-only="1"][data-jms-home-tab-active="0"] .TanadosUI-dot-navigation-container {
  ...
  function isSliderCssActive(cfg) {
    return cfg.enableSlider !== false || matchesAny([
      '#TanadosUI-slides-container',
      '.TanadosUI-slide-progress-bar',
      '.TanadosUI-dot-navigation-container'
    ]);
  }
...
async function scheduleSliderRebuild(reason = "cycle-complete") {
  ...
  try {
    ...
    document.querySelectorAll(".TanadosUI-dot-navigation-container").forEach(n => n.remove());
...
function getSlidesNodeList() {
  ...
  return idxPage ? idxPage.querySelectorAll(".TanadosUI-slide") : null;
}
...
function ensureLayerPropertySanitizer() {
  ...
  const nonPeakSliderTargets = [
    "#TanadosUI-slides-container",
    "#TanadosUI-slides-container .TanadosUI-slide",
    "#TanadosUI-slides-container .TanadosUI-bckdrp-cntnr",
    "#TanadosUI-slides-container .TanadosUI-backdrop",
    "#TanadosUI-slides-container .TanadosUI-horizontal-gradient-overlay",
    "#TanadosUI-slides-container .TanadosUI-horizontal-gradient-overlay:before",
    "#TanadosUI-slides-container .TanadosUI-horizontal-gradient-overlay:after",
    "#TanadosUI-slides-container .TanadosUI-logo-container",
    "#TanadosUI-slides-container .TanadosUI-logo-container .logo-img",
    "#TanadosUI-slides-container .TanadosUI-title-container",
    "#TanadosUI-slides-container .TanadosUI-plot-container",
    "#TanadosUI-slides-container .TanadosUI-provider-container",
    "#TanadosUI-slides-container .TanadosUI-info-container",
    "#TanadosUI-slides-container .TanadosUI-language-container",
    "#TanadosUI-slides-container .TanadosUI-status-container",
    "#TanadosUI-slides-container .TanadosUI-meta-container",
    "#TanadosUI-slides-container .TanadosUI-slider-wrapper",
    "#TanadosUI-slides-container .TanadosUI-button-container",
    "#TanadosUI-slides-container .TanadosUI-button-container *",
    "#TanadosUI-slides-container .TanadosUI-dot-navigation-container",
    "#TanadosUI-slides-container .TanadosUI-dot",
    "#TanadosUI-slides-container .TanadosUI-poster-dot",
    "#TanadosUI-slides-container img.TanadosUI-dot-poster-image"
  ];
...
function upsertSlidesContainerAtTop(indexPage) {
  ...
  let c = indexPage.querySelector("#TanadosUI-slides-container");
  if (!c) {
    c = document.createElement("div");
    c.id = "TanadosUI-slides-container";
  } else {
...
function hydrateFirstSlide(indexPage) {
  if (!indexPage) return;
  const firstActive = indexPage.querySelector(".TanadosUI-slide.active") || indexPage.querySelector(".TanadosUI-slide");
  ...
  const bgCandidates = [
    firstActive.querySelector(".TanadosUI-horizontal-gradient-overlay"),
    firstActive.querySelector(".TanadosUI-slide-backdrop"),
    firstActive.querySelector(".TanadosUI-backdrop"),
...
function ensureInitialActivation(indexPage) {
  if (!indexPage) return;
  const slides = indexPage.querySelectorAll(".TanadosUI-slide");
...
function triggerSlideEnterHooks(indexPage) {
  const active = indexPage.querySelector(".TanadosUI-slide.active") || indexPage.querySelector(".TanadosUI-slide");
  if (!active) return;
  try {
...
function repairVisibleSliderLayout({ forcePrime = false } = {}) {
  ...
  const slides = Array.from(indexPage.querySelectorAll(".TanadosUI-slide"));
  if (!slides.length) return;

  const cfg = (typeof getConfig === "function" ? getConfig() : config) || {};
  const isPeak = !!cfg.peakSlider;
  const slidesContainer = indexPage.querySelector("#TanadosUI-slides-container");
...
function watchActiveSlideChanges() {
  let lastActive = document.querySelector("#indexPage:not(.hide) .TanadosUI-slide.active, #homePage:not(.hide) .TanadosUI-slide.active");
  let resetRafA = 0;
  let resetRafB = 0;
  ...
  const handleChange = (ev) => {
    const eventSlide = ev?.target?.closest?.('.TanadosUI-slide');
    const cur = eventSlide?.classList?.contains('active')
      ? eventSlide
      : document.querySelector("#indexPage:not(.hide) .TanadosUI-slide.active, #homePage:not(.hide) .TanadosUI-slide.active");
...
function warmUpcomingBackdrops(count = 3) {
  try {
    ...
    const slides = [...indexPage.querySelectorAll(".TanadosUI-slide")];
    const active = indexPage.querySelector(".TanadosUI-slide.active") || slides[0];
...
export async function slidesInit() {
  ...
  syncCustomSplashProgress({
    ...
    const createItemAt = async (itemIndex, options = {}) => {
      ...
      if (!isBootActive()) {
        const staleContainer = slideEl?.closest?.("#TanadosUI-slides-container") || null;
        try { slideEl?.__cleanupSlide?.(); } catch {}
        try { slideEl?.remove?.(); } catch {}
        try {
          if (staleContainer && !staleContainer.querySelector(".TanadosUI-slide")) {
    ...
    if (config.peakSlider) {
      ...
      if (!isBootActive()) {
        const staleContainer = firstSlide?.closest?.("#TanadosUI-slides-container") || null;
        ...
        for (const batch of rest) {
          ...
          try {
            ...
            if (config.peakSlider) {
              ...
              const container = idxPage?.querySelector?.('#TanadosUI-slides-container');
...
function initializeSlider(bootToken = Number(window.__jmsSliderBootToken) || 0) {
  try {
    ...
    initSwipeEvents();
    if (config.peakSlider) {
      const sc = indexPage.querySelector('#TanadosUI-slides-container');
      const slides = indexPage.querySelectorAll('.TanadosUI-slide');
      if (sc && slides.length) {
...
function startWhenAllReady() {
  ...
  try {
    const oldDots = document.querySelector(".TanadosUI-dot-navigation-container");
...
if (window.__totalSlidesPlanned > 0 && window.__slidesCreated >= window.__totalSlidesPlanned) {
  ...
  document.addEventListener("jms:all-slides-ready", startWhenAllReady, { once: true });
    ...
    attachMouseEvents();
    const firstImg = indexPage.querySelector(".TanadosUI-slide.active img");
    ...
    indexPage.addEventListener("keydown", async (e) => {
      ...
      if (e.keyCode === 37) {
        ...
        const originEl = focusedSlide.__backdropImg || focusedSlide.querySelector?.(".TanadosUI-backdrop") || focusedSlide;
        try {
    ...
    indexPage.addEventListener("focusin", (e) => {
      if (e.target.closest("#TanadosUI-slides-container")) {
        keyboardActive = true;
        slidesContainer?.classList.remove("disable-interaction");
      }
    });
  ...
  try {
    const active = document.querySelector("#indexPage:not(.hide) .TanadosUI-slide.active, #homePage:not(.hide) .TanadosUI-slide.active");
...
function initializeSliderOnHome({ forceManagedSectionsBoot = false } = {}) {
  const start = async () => {
    ...
    const hasContainer = !!document.querySelector('#indexPage:not(.hide) #TanadosUI-slides-container, #homePage:not(.hide) #TanadosUI-slides-container');
...
function cleanupSlider({ preserveHomeSections = false, invalidateBoot = true, reason = "cleanupSlider" } = {}) {
  ...
  try {
    document
      .querySelectorAll(".TanadosUI-dot-navigation-container, .TanadosUI-slide-progress-seconds")
  ...
  if (host) {
    const sliderContainer = host.querySelector("#TanadosUI-slides-container");
    if (sliderContainer) {
      try {
