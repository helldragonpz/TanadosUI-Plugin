...
export async function getCastAccess({ force = false } = {}) {
  ...
  castAccessPromise = (async () => {
    try {
      const response = await makeCastApiRequest("/Plugins/TanadosUI/cast/access", { __quiet: true });
      const normalized = {
...
async function fetchRemoteGmmpStateMap({ signal } = {}) {
  try {
    const response = await makeCastApiRequest("/Plugins/TanadosUI/gmmp/states", {
      signal,
      __quiet: true
    });
...
async function fetchVisiblePlaybackSessions({ signal } = {}) {
  ...
  try {
    const response = await makeCastApiRequest("/Plugins/TanadosUI/cast/sessions", {
      signal,
      __quiet: true
    });
...
async function sendRemoteGmmpCommand(target, name, args = undefined, { signal, bindToCurrentTrack = true } = {}) {
  ...
  return makeCastApiRequest("/Plugins/TanadosUI/gmmp/commands", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function loadAvailableDevices(itemId, dropdown) {
  dropdown.innerHTML = `<div class="TanadosUI-loading-text">${escapeHtml(t("castyukleniyor", "Cihazlar aranıyor..."))}</div>`;

  try {
    const access = await getCastAccess();
    if (access?.canAccessModule !== true) {
      dropdown.innerHTML = `<div class="TanadosUI-no-devices">${escapeHtml(t("castbulunamadi", "Aygıt bulunamadı"))}</div>`;
      return;
    }
    ...
    if (nowPlayingDevice) {
      ...
      topBanner.className = "TanadosUI-now-playing-banner";
      if (backdropUrl) {
      ...
      topBanner.innerHTML = `
        ...
        ${bannerPosterUrl ? `<img class="TanadosUI-now-playing-poster" src="${escapeHtml(bannerPosterUrl)}" alt="Poster">` : ""}
        <div class="TanadosUI-now-playing-details">
          <div class="TanadosUI-now-playing-title">${renderIcon(getMediaIconClass(nowPlayingItem))} ${escapeHtml(nowPlayingItem.Name || t("castoynatiliyor", "Şu an oynatılıyor"))}</div>
          <div class="TanadosUI-now-playing-device">${escapeHtml(nowPlayingDeviceName)}</div>
          <div class="TanadosUI-now-playing-device">${escapeHtml(nowPlayingDevice.UserName || "")}</div>
        </div>
      `;
      ...
      divider.className = "TanadosUI-cast-divider";
      dropdown.appendChild(divider);
    }

    if (sortedDevices.length === 0) {
      const emptyState = document.createElement("div");
      emptyState.className = "TanadosUI-no-devices";
    ...
    sortedDevices.forEach((device) => {
      ...
      deviceElement.className = "TanadosUI-device-item";
      deviceElement.innerHTML = `
        <div class="TanadosUI-device-icon-container">
          ${getDeviceIcon(deviceClientName)}
        </div>
        <div class="TanadosUI-device-info">
          <div class="TanadosUI-device-name">${escapeHtml(deviceName)}</div>
          <div class="TanadosUI-device-client">${escapeHtml(deviceClientName)}</div>
          ${device.NowPlayingItem ? `<div class="TanadosUI-now-playing">${renderIcon(getMediaIconClass(device.NowPlayingItem))} ${escapeHtml(t("castoynatiliyor", "Şu an oynatılıyor"))}</div>` : ""}
        </div>
      `;
    ...
    dropdown.innerHTML = `<div class="TanadosUI-error-message">${escapeHtml(`${t("casthata", "Hata")}: ${error.message}`)}</div>`;
  }
}
