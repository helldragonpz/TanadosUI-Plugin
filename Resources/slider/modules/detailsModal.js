import { makeApiRequest, fetchItemDetailsFull, getDetailsUrl, playNow, fetchLocalTrailers, pickBestLocalTrailer, getVideoStreamUrl, updateFavoriteStatus, getEmbyHeaders, getSessionInfo } from "../../Plugins/TanadosUI/runtime/api.js";
...
const NESTED_MODAL_SCROLL_ALLOW_SELECTOR = [
  `#${MODAL_ID} .jmsdm-card`,
  `#${WATCHLIST_MODAL_ID}.visible .TanadosUIwl-card`,
  `#${WATCHLIST_MODAL_ID}.visible .TanadosUIwl-share-card`
].join(", ");
...
function ensureLocalCommentStyles() {
  ...
  style.textContent = `
    #${MODAL_ID} .jmsdm-local-comments {
      border-top: 1px solid var(--TanadosUI-border-light);
      margin-top: var(--TanadosUI-gap-xl);
      padding-top: var(--TanadosUI-gap-lg);
    }

    #${MODAL_ID} .jmsdm-local-comments-head {
      margin-bottom: var(--TanadosUI-gap-md);
    }

    #${MODAL_ID} .jmsdm-comments-compose {
      background: linear-gradient(180deg, hsla(0,0%,100%,.05), hsla(0,0%,100%,.03));
      border: 1px solid var(--TanadosUI-border-light);
      border-radius: var(--TanadosUI-radius-card);
      display: flex;
      flex-direction: column;
      gap: var(--TanadosUI-gap-md);
      margin-bottom: var(--TanadosUI-gap-lg);
      padding: var(--TanadosUI-gap-lg);
    }

    #${MODAL_ID} .jmsdm-comments-textarea {
      background: rgba(7,9,15,.52);
      border: 1px solid hsla(0,0%,100%,.08);
      border-radius: var(--TanadosUI-radius-input);
      color: var(--TanadosUI-text-primary);
      font: inherit;
      line-height: 1.6;
      min-height: 110px;
      outline: none;
      padding: 12px 14px;
      resize: vertical;
      transition: border-color var(--TanadosUI-transition-fast), box-shadow var(--TanadosUI-transition-fast);
    }
    ...
    #${MODAL_ID} .jmsdm-comments-textarea::placeholder {
      color: var(--TanadosUI-text-muted);
    }
    ...
    #${MODAL_ID} .jmsdm-comments-compose-meta {
      ...
      display: flex;
      flex-wrap: wrap;
      gap: var(--TanadosUI-gap-sm);
      justify-content: space-between;
    }

    #${MODAL_ID} .jmsdm-comments-hint {
      color: var(--TanadosUI-text-tertiary);
      font-size: 12px;
      line-height: 1.5;
    }
    ...
    #${MODAL_ID} .jmsdm-local-comment-edited {
      ...
      border-radius: var(--TanadosUI-radius-chip);
      color: #ffe1a1;
      font-size: 10px;
      font-weight: 800;
      letter-spacing: .04em;
    ...
    #${MODAL_ID} .jmsdm-local-comment-toolbar {
      ...
      gap: var(--TanadosUI-gap-sm);
      justify-content: space-between;
      margin-top: var(--TanadosUI-gap-sm);
    }
    ...
    #${MODAL_ID} .jmsdm-comment-action {
      ...
      border-radius: var(--TanadosUI-radius-chip);
      color: var(--TanadosUI-text-secondary);
      cursor: pointer;
      font: inherit;
      font-size: 11px;
      font-weight: 800;
      letter-spacing: .03em;
      padding: 6px 10px;
      transition: all var(--TanadosUI-transition-fast);
    }
...
const LOCAL_COMMENTS_ENDPOINT = "/Plugins/TanadosUI/comments";
...
function capturePreviewState() {
  try {
    ...
    const backdropImg = slide.querySelector(".TanadosUI-backdrop, .TanadosUI-backdrop img, .backdrop img, .banner img, img") || null;
