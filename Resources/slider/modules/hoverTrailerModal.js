...
import { playNow, getVideoStreamUrl, fetchItemDetails, fetchPlayableItemDetails, updateFavoriteStatus, goToDetailsPage, fetchLocalTrailers, pickBestLocalTrailer, getCachedUserTopGenres } from '../../Plugins/TanadosUI/runtime/api.js';
...
function isInsideDotArea(node) {
  return !!(node?.closest?.('.TanadosUI-dot-navigation-container') || node?.closest?.('.TanadosUI-poster-dot'));
}
...
export async function updateModalContent(item, videoUrl) {
  ...
  const contextIsDot = modalState._modalContext === 'TanadosUI-dot';
...
export function createVideoModal({ showButtons = true, context = 'TanadosUI-dot' } = {}) {
  if (!config) return null;
  const allow = (context === 'TanadosUI-dot')
  ...
  favoriteButton.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const slide = document.querySelector(`.TanadosUI-slide[data-item-id="${itemId}"]`);
      if (slide) {
...
export function animatedOpen(modal, anchorEl, pos = 'item') {
  ...
  else if (pos === 'TanadosUI-dot') positionModalRelativeToDot(modal, anchorEl);
  animatedShow(modal);
}
