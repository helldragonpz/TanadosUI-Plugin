...
async function fetchJmsConfig() {
  const response = await fetch("/TanadosUI/config", {
    method: "GET",
    cache: "no-store",
  ...
  setSharedBackendMode("TanadosUI");
...
export function getRadioPersistenceInfo() {
  return {
    ...
    supportsServerWrite: sharedBackendMode === "TanadosUI"
  };
}
...
export async function fetchSharedRadioStations() {
  try {
    ...
    if (sharedBackendMode !== "TanadosUI") {
