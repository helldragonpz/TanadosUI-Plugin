...
import { getEmbyHeaders, getSessionInfo } from "../../../Plugins/TanadosUI/runtime/api.js";
...
async function postRemoteGmmpState(payload, { keepalive = false } = {}) {
  ...
  try {
    const response = await fetch(apiUrl("/Plugins/TanadosUI/gmmp/state"), {
      method: "POST",
...
async function pollRemoteGmmpCommands() {
  ...
  try {
    const url = new URL(apiUrl("/Plugins/TanadosUI/gmmp/commands"));
    if (sessionId) {
...
function ensurePointerStylesInjected() {
  ...
  a.TanadosUI-watchlist-nav-button.TanadosUI-watchlist-nav-link.MuiButtonBase-root.MuiButton-root.MuiButton-text.MuiButton-textInherit.MuiButton-sizeMedium.MuiButton-textSizeMedium.MuiButton-colorInherit {
      display: inline-flex;
      -webkit-box-align: center;
      align-items: center;
      -webkit-box-pack: center;
  ...
  span.MuiButton-icon.MuiButton-startIcon.MuiButton-iconSizeMedium.TanadosUI-watchlist-nav-icon {
      font-size: 18px;
  }
  `;
