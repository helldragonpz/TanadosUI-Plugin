...
export function updateSlidePosition() {
  ...
  const slidesContainer = document.querySelector("#TanadosUI-slides-container");
  if (slidesContainer) {
  ...
  containerTypes.forEach(type => {
    document.querySelectorAll(`.TanadosUI-${type}-container`).forEach(container => {
      applyContainerStyles(container, type);
    });
  });

  const sliderWrapper = document.querySelector(".TanadosUI-slider-wrapper");
  if (sliderWrapper) applyContainerStyles(sliderWrapper, 'slider');

  const progressBar = document.querySelector(".TanadosUI-slide-progress-bar");
  if (progressBar) applyContainerStyles(progressBar, 'progress');

  const progressSeconds = document.querySelector(".TanadosUI-slide-progress-seconds");
