...
function openLyricsModal(labels, opts = {}) {
  ...
  (async () => {
    try {
      const r = await fetch('/TanadosUI/lyrics/status', { headers: getJFHeaders() });
...
async function startLyricsJob(labels, refs) {
  ...
  try {
    const r = await fetch('/TanadosUI/lyrics/run', {
      method: 'POST',
...
async function cancelLyricsJob(labels) {
  ...
  try {
    await fetch('/TanadosUI/lyrics/cancel', { method: 'POST', headers: getJFHeaders() });
  } catch {}
