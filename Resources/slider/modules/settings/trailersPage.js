...
export function createTrailersPanel(config, labels) {
  ...
  async function connectIfRunning({ forceOpen = false } = {}) {
    try {
      ...
      const res = await fetch('/TanadosUI/trailers/status', { method: 'GET', headers });
  ...
  modal.btnOk.onclick = async () => {
    ...
    try {
      ...
      const headers = await getAuthHeaders();
      const res = await fetch('/TanadosUI/trailers/run', { method: 'POST', headers, body: JSON.stringify(body) });
  ...
  modal.btnStop.onclick = async () => {
    ...
    try {
      const headers = await getAuthHeaders();
      await fetch('/TanadosUI/trailers/cancel', { method: 'POST', headers });
      startPolling();
  ...
  async function loadLatestTrailerConfig() {
    const headers = await getAuthHeaders();
    const res = await fetch('/TanadosUI/config', { method: 'GET', headers });
    ...
    function renderAdminFields() {
      ...
      saveBtn.onclick = async () => {
        ...
        try {
          ...
          const headers = await getAuthHeaders();
          const res = await fetch('/TanadosUI/config', { method: 'POST', headers, body: JSON.stringify(payload) });
          const txt = await res.text();
          let data = {};
