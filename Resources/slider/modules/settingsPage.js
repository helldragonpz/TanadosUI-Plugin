...
export function createSettingsModal() {
    ...
    title.textContent = TanadosUITabLabel;
    ...
    const mainTab = createTab('TanadosUI', 'fa-sliders', TanadosUITabLabel, true);
...
function activateSettingsPanel(modal, tab = 'TanadosUI') {
    ...
    const fallbackTab = modal.querySelector('.settings-tab[data-tab="TanadosUI"]')
        || modal.querySelector('.settings-tab[data-tab="slider"]');
    const fallbackPanel = modal.querySelector('#TanadosUI-panel')
...
function createMainSettingsPanel(labels, panels) {
    ...
    const homeSectionsMaster = createCheckbox(
        ...
        labels.enableHomeSectionsMaster || 'TanadosUI ui kartlarını etkinleştir',
    ...
    enablesSection.appendChild(createTextInput(
        ...
        config.customSplashTitle || labels.customSplashTitle || 'TanadosUI'
    ));
...
export function initSettings(defaultTab = 'TanadosUI') {
...
export function mountTanadosUISettingsPage(host, { defaultTab = 'TanadosUI', force = false } = {}) {
    if (!host) return null;
    ...
    host.__jmsTanadosUISettingsApi = api;
    host.__jmsTanadosUIApi = api;
    api.open(defaultTab);
    return api;
}
