...
import { updateFavoriteStatus, updatePlayedStatus, fetchItemDetails, getSessionInfo } from "../../Plugins/TanadosUI/runtime/api.js";
...
(function injectBackdropPerfStyles(){
  ...
  st.textContent = `
    ...
    .TanadosUI-backdrop.is-lqip { filter: blur(14px); transform: scale(1.02); }
    .TanadosUI-backdrop.is-hi-pending { opacity: .94; }
    #TanadosUI-slides-container.is-scrolling *,
    #TanadosUI-slides-container.is-scrolling .TanadosUI-backdrop,
    #TanadosUI-slides-container.is-scrolling .TanadosUI-horizontal-gradient-overlay {
...
function isPeakBackdropEligible(img) {
  ...
  const slide = img.closest?.('.TanadosUI-slide');
...
async function createSlide(item, options = {}) {
  ...
  const existing = slidesContainer.querySelector(`.TanadosUI-slide[data-item-id="${itemIdRaw}"]`);
  (function bindSlidesScrollPerf(){
    if (window.__jmsSlidesScrollBound) return;
    window.__jmsSlidesScrollBound = true;
    const scroller = document.querySelector('#TanadosUI-slides-container');
    if (!scroller) return;
    const onScrollPerf = () => {
    ...
    const mo = new MutationObserver((muts) => {
      for (const m of muts) {
        m.removedNodes?.forEach?.(node => {
          ...
          node?.querySelectorAll?.('.TanadosUI-slide')?.forEach(el => {
  ...
  const existingSlides = Array.from(slidesContainer.children).filter((child) => child.classList?.contains("TanadosUI-slide"));
  ...
  slide.className = "TanadosUI-slide";
  ...
  backdropImg.className = 'TanadosUI-backdrop';
  ...
  const pinActiveIfNeeded = () => {
    const slideEl = backdropImg.closest('.TanadosUI-slide');
  ...
  backdropImg.addEventListener('click', async (ev) => {
    const slideEl = ev.currentTarget.closest('.TanadosUI-slide');
  ...
  backdropContainer.className = 'TanadosUI-bckdrp-cntnr';
  ...
  function createLogoElement(fallback) {
    ...
    logoImg.className = "TanadosUI-logo";
    logoImg.src = logoUrl;
    logoImg.alt = "";
  ...
  function createDiskElement(fallback) {
    ...
    discImg.className = "TanadosUI-disk";
    discImg.src = discUrl;
    discImg.alt = "";
  ...
  function createTitleElement() {
    ...
    titleDiv.className = "TanadosUI-no-logo-container";
  ...
  const slideChildren = Array.from(slidesContainer.children).filter((child) => child.classList?.contains("TanadosUI-slide"));
  const dotNav = Array.from(slidesContainer.children).find((child) => child.classList?.contains("TanadosUI-dot-navigation-container")) || null;
...
function buildStarLayer(useSolid) {
  ...
  layer.className = useSolid ? "TanadosUI-trailer-star-fill" : "TanadosUI-trailer-star-track";
...
function openTrailerModal(trailerUrl, trailerName, itemName = '', itemType = '', isFavorite = false, itemId = null, updateFavoriteCallback = null, CommunityRating = null, CriticRating = null, OfficialRating = null) {
  ...
  overlay.className = "TanadosUI-trailer-modal-overlay";
  overlay.style.opacity = "0";
  overlay.style.transition = "opacity 0.3s ease-in-out";

  const modal = document.createElement("div");
  modal.className = "TanadosUI-trailer-modal";
  modal.style.maxWidth = "90vw";
  modal.style.maxHeight = "90vh";
  modal.style.width = "800px";

  const modalHeader = document.createElement("div");
  modalHeader.className = "TanadosUI-trailer-modal-header";

  const logoContainer = document.createElement("div");
  logoContainer.className = "TanadosUI-trailer-modal-logo";
  ...
  closeBtn.className = "TanadosUI-trailer-modal-close";
  ...
  videoContainer.className = "TanadosUI-trailer-video-container";
  videoContainer.style.paddingBottom = "56.25%";

  const loadingSpinner = document.createElement("div");
  loadingSpinner.className = "TanadosUI-trailer-loading";
 ...
 const playViaAPI = () => {
  ...
  modalFooter.className = "TanadosUI-trailer-modal-footer";
  ...
  if (CommunityRating != null) {
    ...
    starWrap.className = "TanadosUI-trailer-star-rating";
  ...
  if (!document.getElementById('trailer-modal-styles')) {
    ...
    style.textContent = `
      .TanadosUI-trailer-modal-header {
        display: flex;
        align-items: center;
      ...
      .TanadosUI-trailer-modal-logo {
        flex-shrink: 0;
      }

      .TanadosUI-trailer-loading {
        position: absolute;
        top: 50%;
        left: 50%;
      ...
      @media (max-width: 768px) {
        .TanadosUI-trailer-modal {
          width: 95vw !important;
          height: auto !important;
        }

        .TanadosUI-trailer-modal-header h3 {
          font-size: 14px;
