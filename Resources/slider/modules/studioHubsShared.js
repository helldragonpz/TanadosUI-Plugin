...
export async function fetchStudioHubVideoEntries({ force = false } = {}) {
  ...
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/video${force ? `?ts=${Date.now()}` : ""}`), {
    method: "GET",
    cache: "no-store",
    headers
  });
...
export async function fetchStudioHubManualEntries({ force = false } = {}) {
  const headers = await getAuthHeaders();
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/collection${force ? `?ts=${Date.now()}` : ""}`), {
    method: "GET",
    cache: "no-store",
    headers
  });
...
export function buildStudioHubVideoUrl(entry) {
  ...
  return withServer(`/Plugins/TanadosUI/studio-hubs/video/${encodeURIComponent(fileName)}?v=${encodeURIComponent(updatedAt)}`);
}
...
export function buildStudioHubLogoUrl(entry) {
  ...
  if (!fileName) return null;
  const updatedAt = Number(entry?.updatedAtUtc || entry?.UpdatedAtUtc || Date.now());
  return withServer(`/Plugins/TanadosUI/studio-hubs/logo/${encodeURIComponent(fileName)}?v=${encodeURIComponent(updatedAt)}`);
}
...
export async function fetchStudioHubVisibility({ force = false, profile } = {}) {
  ...
  const headers = await getAuthHeaders();
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/visibility?profile=${encodeURIComponent(normalizedProfile)}&ts=${Date.now()}`), {
    method: "GET",
    headers,
    cache: "no-store"
  });
...
export async function saveStudioHubVisibility(hiddenNames, { profile, orderNames } = {}) {
  ...
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/visibility?profile=${encodeURIComponent(normalizedProfile)}`), {
    method: "POST",
    headers,
    body: JSON.stringify({
...
export async function createStudioHubManualEntry({ studioId, name }) {
  ...
  const res = await fetch(withServer("/Plugins/TanadosUI/studio-hubs/collection"), {
    method: "POST",
    headers,
...
export async function deleteStudioHubManualEntry(studioId) {
  ...
  const headers = await getAuthHeaders();
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/collection?studioId=${encodeURIComponent(cleanStudioId)}`), {
    method: "DELETE",
    headers,
    cache: "no-store"
  });
...
export async function uploadStudioHubLogo(studioId, file) {
  ...
  const res = await fetch(withServer("/Plugins/TanadosUI/studio-hubs/logo"), {
    method: "POST",
    headers,
    body: formData,
    cache: "no-store"
...
export async function deleteStudioHubLogo(studioId) {
  ...
  const headers = await getAuthHeaders();
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/logo?studioId=${encodeURIComponent(cleanStudioId)}`), {
    method: "DELETE",
    headers,
    cache: "no-store"
  });
...
export async function uploadStudioHubVideo(name, file) {
  ...
  const res = await fetch(withServer("/Plugins/TanadosUI/studio-hubs/video"), {
    method: "POST",
    headers,
    body: formData,
    cache: "no-store"
...
export async function deleteStudioHubVideo(name) {
  ...
  const headers = await getAuthHeaders();
  const res = await fetch(withServer(`/Plugins/TanadosUI/studio-hubs/video?name=${encodeURIComponent(cleanName)}`), {
    method: "DELETE",
    headers,
    cache: "no-store"
  });
