import { fetchItemDetailsFull, fetchItemsBulk, getEmbyHeaders, getLastPlayNowBlockReason, getSessionInfo, makeApiRequest, playNow, updateFavoriteStatus } from "../../Plugins/TanadosUI/runtime/api.js";
...
const WATCHLIST_STYLE_ID = "TanadosUI-watchlist-modal-style";
const WATCHLIST_NAV_BUTTON_CLASS = "TanadosUI-watchlist-nav-button";
const WATCHLIST_MUI_NAV_LINK_CLASS = "TanadosUI-watchlist-nav-link";
const WATCHLIST_NAV_KIND_ATTR = "data-TanadosUI-watchlist-nav-kind";
...
function setStudioHubLoadingState(targetEl, isLoading) {
  const el = targetEl?.closest?.("[data-TanadosUIwl-studio-id]") || targetEl;
  if (!el) return false;
...
function getWatchlistTabsButtonMarkup(label) {
  ...
  return `
    <span class="TanadosUI-watchlist-nav-icon" aria-hidden="true">
      <svg class="TanadosUI-watchlist-nav-svg" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20" focusable="false">
...
function getWatchlistMuiTabsButtonMarkup(label) {
  const safeLabel = escapeHtml(label);
  return `
    <span class="MuiButton-icon MuiButton-startIcon MuiButton-iconSizeMedium TanadosUI-watchlist-nav-icon" aria-hidden="true">
      <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium TanadosUI-watchlist-nav-svg" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" focusable="false" aria-hidden="true" viewBox="0 0 20 20">
        <path fill="currentColor" d="${WATCHLIST_ICON_PATH}" />
      </svg>
    </span>
    <span class="TanadosUI-watchlist-nav-label">${safeLabel}</span>
...
async function syncFavoritesOnStartup() {
  ...
  const storageKey = `TanadosUI:watchlist:favorites-bootstrap:${serverId || "default"}:${userId || "anonymous"}`;
...
function installJellyfinFavoriteMirror() {
  ...
  if (typeof XMLHttpRequest !== "undefined") {
    ...
    proto.open = function patchedWatchlistFavoriteMirrorOpen(method, url, ...rest) {
      this.__TanadosUIFavoriteMirror = {
        method: text(method),
        url: extractRequestUrl(url)
      };
      this.__TanadosUIFavoriteMirrorListenerAttached = false;
      return nativeOpen.call(this, method, url, ...rest);
    };

    proto.send = function patchedWatchlistFavoriteMirrorSend(body) {
      if (!this.__TanadosUIFavoriteMirrorListenerAttached) {
        this.__TanadosUIFavoriteMirrorListenerAttached = true;
        this.addEventListener("loadend", () => {
          if (this.status < 200 || this.status >= 300) return;
          const details = this.__TanadosUIFavoriteMirror || {};
...
function buildWatchlistHeaders(extra = {}) {
  ...
  if (userName) headers["X-TanadosUI-UserName"] = userName;
...
function patchItemMembership(item) {
  ...
  item.__TanadosUIInWatchlist = inWatchlist;
  return item;
}
...
function notifyWatchlistChanged(detail = {}) {
  try {
    window.dispatchEvent(new CustomEvent("TanadosUI:watchlist-changed", {
      detail: {
        ...detail,
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-backdrop {
      position: absolute;
      inset: 0;
      background:
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-card {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-header {
      display: flex;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-title {
      font-size: 28px;
      font-weight: 800;
      letter-spacing: -0.03em;
      margin: 0 0 6px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-subtitle {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-header-actions {
      display: flex;
      align-items: center;
      gap: 10px;
      flex-shrink: 0;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-close,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-tab,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-btn,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-submit,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-cancel {
      border: 0;
      cursor: pointer;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-close:hover,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-btn:hover,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-submit:hover,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-cancel:hover {
    ...
    .emby-tabs-slider .${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-icon,
    .${WATCHLIST_MUI_NAV_LINK_CLASS}.${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-icon {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      line-height: 1;
    ...
    .emby-tabs-slider .${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-svg,
    .${WATCHLIST_MUI_NAV_LINK_CLASS}.${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-svg {
      display: block;
      width: 1em;
      height: 1em;
      min-width: 1em;
    ...
    .emby-tabs-slider .${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-label,
    .${WATCHLIST_MUI_NAV_LINK_CLASS}.${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-label {
      display: inline-block;
      pointer-events: none;
    }
    .emby-tabs-slider .${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-icon svg,
    .emby-tabs-slider .${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-icon path,
    .${WATCHLIST_MUI_NAV_LINK_CLASS}.${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-icon svg,
    .${WATCHLIST_MUI_NAV_LINK_CLASS}.${WATCHLIST_NAV_BUTTON_CLASS} .TanadosUI-watchlist-nav-icon path {
      pointer-events: none;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-tabs {
      display: inline-flex;
      gap: 10px;
      padding: 14px 24px 0;
      flex-wrap: wrap;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-tab.active {
      background: linear-gradient(135deg, #ffb703, #fb8500);
      color: #141822;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-body {
      flex: 1;
      min-height: 0;
      overflow: hidden;
      padding: 18px 24px 24px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-layout {
      display: grid;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-layout.is-stats-tab {
      grid-template-columns: minmax(0, 1fr);
      grid-template-areas: "main";
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-layout.is-stats-tab .TanadosUIwl-preview {
      display: none;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-main {
      grid-area: main;
      min-height: 0;
      overflow: auto;
      padding-right: 6px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-layout.is-stats-tab .TanadosUIwl-main {
      padding-right: 0;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-shell {
      display: flex;
      flex-direction: column;
      gap: 18px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero {
      position: relative;
      isolation: isolate;
      display: grid;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-content {
      position: relative;
      z-index: 1;
      min-width: 0;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art {
      position: relative;
      display: flex;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art::before {
      content: "";
      position: absolute;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art::after {
      content: "";
      position: absolute;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-badge {
      position: relative;
      z-index: 1;
      display: inline-flex;
      align-items: center;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-icon {
      width: 46px;
      height: 46px;
      filter: drop-shadow(0 8px 16px rgba(0,0,0,0.24));
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-bars {
      position: absolute;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-bars span {
      display: block;
      height: 10px;
      border-radius: 999px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-bars span:nth-child(1) {
      width: 100%;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-bars span:nth-child(2) {
      width: 78%;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-art-bars span:nth-child(3) {
      width: 58%;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-kicker {
      color: #ffb703;
      font-size: 11px;
      font-weight: 800;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-user {
      margin: 10px 0 8px;
      font-size: 30px;
      line-height: 1.05;
      font-weight: 900;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-copy {
      margin: 0;
      max-width: 560px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-total {
      margin-top: 22px;
      display: inline-flex;
      flex-direction: column;
      gap: 6px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-total-label {
      color: rgba(255,255,255,0.72);
      font-size: 12px;
      font-weight: 700;
      letter-spacing: 0.04em;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-total-value {
      color: #fff7e0;
      font-size: 46px;
      line-height: 1;
      font-weight: 900;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-cards {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(170px, 1fr));
      gap: 12px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stat-card {
      min-height: 116px;
      display: flex;
      flex-direction: column;
      justify-content: space-between;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stat-label {
      color: rgba(255,255,255,0.72);
      font-size: 12px;
      line-height: 1.5;
      font-weight: 700;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stat-value {
      color: #fff;
      font-size: 34px;
      line-height: 1;
      font-weight: 900;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-breakdown {
      border-radius: 22px;
      padding: 18px;
      border: 1px solid rgba(255,255,255,0.06);
      background:
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-breakdown-head {
      margin-bottom: 14px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-breakdown-title {
      margin: 0;
      font-size: 18px;
      line-height: 1.2;
      font-weight: 800;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-grid {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(190px, 1fr));
      gap: 12px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-card {
      display: flex;
      flex-direction: column;
      gap: 10px;
      padding: 16px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-name {
      font-size: 16px;
      font-weight: 800;
      line-height: 1.25;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-total,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-row {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 12px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-total strong,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-type-row strong {
      color: #fff;
      font-size: 16px;
      font-weight: 800;
      line-height: 1;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-page {
      display: flex;
      flex-direction: column;
      gap: 18px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-stats-hero-secondary {
      background:
        radial-gradient(circle at top left, rgba(96,165,250,0.16), transparent 36%),
        linear-gradient(135deg, rgba(18,22,32,0.98), rgba(10,12,18,0.98));
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-grid {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
      gap: 12px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card {
      position: relative;
      isolation: isolate;
      overflow: hidden;
      display: flex;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card::before {
      content: "";
      position: absolute;
      right: -8px;
      bottom: -4px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card[data-media-key="movies"] {
      background:
        radial-gradient(circle at top right, rgba(244,63,94,0.16), transparent 40%),
        rgba(255,255,255,0.04);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card[data-media-key="series"] {
      background:
        radial-gradient(circle at top right, rgba(96,165,250,0.16), transparent 40%),
        rgba(255,255,255,0.04);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card[data-media-key="music"] {
      background:
        radial-gradient(circle at top right, rgba(52,211,153,0.16), transparent 40%),
        rgba(255,255,255,0.04);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-head {
      position: relative;
      z-index: 1;
      display: flex;
      align-items: center;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-badge {
      flex: 0 0 auto;
      width: 40px;
      height: 40px;
      display: inline-flex;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-badge svg {
      width: 20px;
      height: 20px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-title {
      margin: 0;
      font-size: 18px;
      line-height: 1.2;
      font-weight: 800;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-stats {
      position: relative;
      z-index: 1;
      display: grid;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-stat {
      padding: 12px;
      border-radius: 14px;
      background: rgba(255,255,255,0.04);
      border: 1px solid rgba(255,255,255,0.05);
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-stat span,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-last-label,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-last-subtitle,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-last-meta,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-repeat-subtitle {
      color: rgba(255,255,255,0.72);
      font-size: 12px;
      line-height: 1.45;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-card-stat strong,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-repeat-count {
      color: #fff;
      font-size: 24px;
      line-height: 1;
      font-weight: 900;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-last {
      position: relative;
      z-index: 1;
      margin-top: auto;
      display: flex;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-last-title,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-repeat-title {
      font-size: 15px;
      line-height: 1.35;
      font-weight: 800;
      color: #fff;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-last-empty {
      color: rgba(255,255,255,0.6);
      font-size: 13px;
      line-height: 1.5;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-repeat-list {
      display: flex;
      flex-direction: column;
      gap: 10px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-repeat-item {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 14px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-general-repeat-main {
      min-width: 0;
      display: flex;
      flex-direction: column;
      gap: 4px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview {
      grid-area: preview;
      min-height: 0;
      overflow: auto;
      border-radius: 22px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-empty {
      min-height: 100%;
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-empty-copy {
      max-width: 280px;
      line-height: 1.7;
      font-size: 13px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-shell {
      display: flex;
      flex-direction: column;
      min-height: 100%;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-hero {
      position: relative;
      min-height: 252px;
      overflow: hidden;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-backdrop {
      position: absolute;
      inset: 0;
      width: 100%;
      height: 100%;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-hero::after {
      content: "";
      position: absolute;
      inset: 0;
      background:
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-hero-inner {
      position: relative;
      z-index: 1;
      display: grid;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-poster {
      width: 104px;
      height: 152px;
      border-radius: 16px;
      overflow: hidden;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-poster img {
      width: 100%;
      height: 100%;
      object-fit: cover;
      display: block;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-poster-fallback {
      width: 100%;
      height: 100%;
      display: flex;
      align-items: flex-end;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-head {
      min-width: 0;
      display: flex;
      flex-direction: column;
      gap: 10px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-kicker {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-title {
      margin: 0;
      font-size: 24px;
      line-height: 1.08;
      font-weight: 900;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-subtitle {
      color: rgba(255,255,255,0.74);
      font-size: 13px;
      line-height: 1.5;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-chips {
      display: flex;
      flex-wrap: wrap;
      gap: 8px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-chip {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      border-radius: 6px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-chip.accent {
      background: linear-gradient(135deg, rgba(255,183,3,0.22), rgba(251,133,0,0.18));
      color: #fff3d2;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-progress {
      margin-top: 2px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-progress-track {
      width: 100%;
      height: 8px;
      border-radius: 6px;
      overflow: hidden;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-progress-bar {
      height: 100%;
      border-radius: inherit;
      background: linear-gradient(90deg, #ffb703, #fb8500);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-progress-copy {
      margin-top: 8px;
      color: rgba(255,255,255,0.72);
      font-size: 12px;
      line-height: 1.5;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-body {
      padding: 18px;
      display: flex;
      flex-direction: column;
      gap: 16px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-loading {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-note {
      margin: 0;
      padding: 12px 14px;
      border-radius: 16px;
      background: rgba(255,255,255,0.04);
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-overview {
      margin: 0;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-stats {
      display: grid;
      grid-template-columns: repeat(2, minmax(0, 1fr));
      gap: 10px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-stat {
      display: flex;
      flex-direction: column;
      gap: 6px;
      padding: 12px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-stat-label {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-stat-value {
      color: #fff;
      font-size: 13px;
      line-height: 1.5;
      font-weight: 700;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-section {
      display: flex;
      flex-direction: column;
      gap: 10px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-section-title {
      margin: 0;
      color: rgba(255,255,255,0.66);
      font-size: 11px;
      font-weight: 800;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-field-list {
      display: grid;
      gap: 8px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-field {
      display: grid;
      grid-template-columns: 90px minmax(0, 1fr);
      gap: 10px;
      align-items: start;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-field-label {
      color: rgba(255,255,255,0.52);
      font-size: 12px;
      font-weight: 700;
      line-height: 1.45;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-field-value {
      color: rgba(255,255,255,0.88);
      font-size: 12px;
      line-height: 1.6;
      word-break: break-word;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-list {
      list-style: none;
      margin: 0;
      padding: 0;
      display: grid;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-list li {
      padding: 10px 12px;
      border-radius: 8px;
      background: rgba(255,255,255,0.04);
      border: 1px solid rgba(255,255,255,0.06);
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-tags {
      display: flex;
      flex-wrap: wrap;
      gap: 8px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-tag {
      appearance: none;
      border-radius: 6px;
      background: rgba(255,255,255,0.08);
      border: 1px solid rgba(255,255,255,0.06);
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-tag-button {
      cursor: pointer;
      transition: background 140ms ease, border-color 140ms ease, color 140ms ease, transform 140ms ease;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-tag-button:hover {
      background: rgba(255,183,3,0.16);
      border-color: rgba(255,183,3,0.28);
      color: #fff3d2;
      transform: translateY(-1px);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-tag-button:focus-visible {
      outline: 2px solid rgba(255,183,3,0.55);
      outline-offset: 2px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-tag-button.is-copied {
      background: linear-gradient(135deg, rgba(255,183,3,0.24), rgba(251,133,0,0.18));
      border-color: rgba(255,183,3,0.36);
      color: #fff6dd;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-head {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 10px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-count {
      color: rgba(255,255,255,0.62);
      font-size: 11px;
      font-weight: 700;
      line-height: 1.4;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-grid {
      display: grid;
      grid-template-columns: repeat(2, minmax(0, 1fr));
      gap: 10px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-card {
      appearance: none;
      display: block;
      width: 100%;
      min-width: 0;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-card:hover,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-card:focus-visible {
      transform: translateY(-1px);
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-poster {
      position: relative;
      overflow: hidden;
      border-radius: 12px;
      aspect-ratio: 2 / 3;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-poster img {
      width: 100%;
      height: 100%;
      object-fit: cover;
      display: block;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-fallback {
      width: 100%;
      height: 100%;
      display: flex;
      align-items: flex-end;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-main {
      min-width: 0;
      display: flex;
      flex-direction: column;
      gap: 6px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-title {
      color: #fff;
      font-size: 12px;
      font-weight: 800;
      line-height: 1.45;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-meta {
      color: rgba(255,255,255,0.66);
      font-size: 11px;
      line-height: 1.45;
      display: -webkit-box;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-progress {
      height: 5px;
      border-radius: 999px;
      overflow: hidden;
      background: rgba(255,255,255,0.10);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-progress-bar {
      height: 100%;
      border-radius: inherit;
      background: linear-gradient(90deg, #ffb703, #fb8500);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-note {
      color: rgba(255,255,255,0.62);
      font-size: 12px;
      line-height: 1.5;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-played-overlay {
      position: absolute;
      inset: 0;
      display: flex;
      flex-direction: column;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-played-mark {
      width: 60px;
      height: 60px;
      border-radius: 999px;
      display: inline-flex;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-played-mark svg {
      width: 34px;
      height: 34px;
      display: block;
      fill: currentColor;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-played-text {
      color: #f4fff7;
      font-size: 10px;
      font-weight: 900;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item.is-played,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-card.is-played {
      border-color: rgba(129,201,149,0.34);
      box-shadow: 0 14px 30px rgba(0, 0, 0, 0.18), inset 0 0 0 1px rgba(163,230,53,0.08);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item.is-played .TanadosUIwl-item-poster img,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-preview-collection-card.is-played .TanadosUIwl-preview-collection-poster img {
      filter: saturate(0.85) brightness(0.76);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-poster .TanadosUIwl-played-mark {
      width: 54px;
      height: 54px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-poster .TanadosUIwl-played-mark svg {
      width: 30px;
      height: 30px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-poster .TanadosUIwl-played-text {
      font-size: 9px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-section + .TanadosUIwl-section {
      margin-top: 22px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-section-head {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 14px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-section-title {
      margin: 0;
      font-size: 16px;
      font-weight: 800;
      letter-spacing: -0.02em;
    }
   #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-sharemeta {
      background: linear-gradient(135deg, #ffb703, #fb8500) !important;
      -webkit-background-clip: text !important;
      background-clip: text !important;
      color: transparent !important;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-grid {
      display: grid;
      grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
      gap: 14px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item:hover,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item.is-preview-active,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item:focus-within {
      transform: translateY(-2px);
      border-color: rgba(255, 183, 3, 0.42);
      box-shadow: 0 14px 30px rgba(0, 0, 0, 0.18);
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-poster {
      background:
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-poster img {
      width: 100%;
      height: 100%;
      object-fit: cover;
      display: block;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-poster-fallback {
      inset: 0;
      position: absolute;
      display: flex;
      align-items: flex-end;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-main {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-title {
      margin: 0;
      font-size: 17px;
      font-weight: 800;
      line-height: 1.22;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-meta,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-extra,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-sharemeta {
      color: rgba(255,255,255,0.72);
      font-size: 12px;
      line-height: 1.5;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-overview {
      color: rgba(255,255,255,0.78);
      font-size: 13px;
      line-height: 1.56;
      display: -webkit-box;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-tags {
      display: flex;
      gap: 8px;
      flex-wrap: wrap;
      align-items: center;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-tag {
      border-radius: 6px;
      background: rgba(255,255,255,0.08);
      color: rgba(255,255,255,0.84);
      font-size: 11px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item-actions {
      display: flex;
      gap: 8px;
      flex-wrap: wrap;
      margin-top: auto;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-btn.primary {
      background: linear-gradient(135deg, #ffb703, #fb8500);
      color: #1b1f28;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-btn.danger {
      background: rgba(244, 63, 94, 0.18);
      color: #ffd7df;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-item:focus {
      outline: none;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-empty,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-loading,
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-error {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-overlay {
      position: absolute;
      inset: 0;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-card {
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-title {
      margin: 0 0 6px;
      font-size: 22px;
      font-weight: 800;
      letter-spacing: -0.03em;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-help {
      color: rgba(255,255,255,0.7);
      font-size: 13px;
      line-height: 1.56;
      margin: 0 0 16px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-list {
      display: grid;
      gap: 8px;
      margin-bottom: 16px;
      max-height: 240px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-user {
      display: flex;
      align-items: center;
      gap: 10px;
      background: rgba(255,255,255,0.04);
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-user input {
      accent-color: #ffb703;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-note-label {
      display: block;
      font-size: 13px;
      font-weight: 700;
      margin-bottom: 8px;
    }
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-note {
      width: 100%;
      min-height: 120px;
      resize: vertical;
      border-radius: 8px;
    ...
    #${WATCHLIST_MODAL_ID} .TanadosUIwl-share-footer {
      display: flex;
      gap: 10px;
      justify-content: flex-end;
      margin-top: 16px;
...
function ensureModalRoot() {
  ...
  root.addEventListener("click", async (event) => {
    const closeButton = event.target?.closest?.("[data-TanadosUIwl-close='1']");
    if (closeButton) {
      await closeWatchlistModal();
      return;
    }

    const backdrop = event.target?.closest?.(".TanadosUIwl-backdrop");
    const card = event.target?.closest?.(".TanadosUIwl-card");
    if (backdrop && !card) {
      await closeWatchlistModal();
    }
  });

  window.addEventListener("TanadosUI:watchlist-changed", (event) => {
...
function renderPlayedOverlayMarkup() {
  return `
    <div class="TanadosUIwl-played-overlay" aria-hidden="true">
      <span class="TanadosUIwl-played-mark">
        <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
          <path d="M9.2 16.6 4.8 12.2 3.4 13.6 9.2 19.4 20.6 8 19.2 6.6z"></path>
        </svg>
      </span>
      <span class="TanadosUIwl-played-text">${escapeHtml(L("played", "İzlendi"))}</span>
    </div>
  `;
}
...
function renderCollectionPreviewCards(items = [], { mode = "collection" } = {}) {
  ...
  return `
    <div class="TanadosUIwl-preview-collection-grid">
      ${visible.map((item) => {
        ...
        return `
          <button
            type="button"
            class="TanadosUIwl-preview-collection-card ${isPlayed ? "is-played" : ""}"
            data-TanadosUIwl-preview-play="${escapeHtml(item?.Id)}"
            aria-label="${escapeHtml(`${playLabel}: ${title}`)}"
          >
            <div class="TanadosUIwl-preview-collection-poster">
              ${posterUrl
                ? `<img src="${escapeHtml(posterUrl)}" alt="${escapeHtml(title)}" loading="lazy" decoding="async">`
                : `<div class="TanadosUIwl-preview-collection-fallback">${escapeHtml(fallback)}</div>`}
              ${isPlayed ? renderPlayedOverlayMarkup() : ""}
            </div>
            <div class="TanadosUIwl-preview-collection-main">
              <div class="TanadosUIwl-preview-collection-title">${escapeHtml(title)}</div>
              ${meta ? `<div class="TanadosUIwl-preview-collection-meta">${escapeHtml(meta)}</div>` : ""}
              ${progressPercent > 0 ? `
                <div class="TanadosUIwl-preview-collection-progress">
                  <div class="TanadosUIwl-preview-collection-progress-bar" style="width:${progressPercent}%"></div>
                </div>
              ` : ""}
            </div>
          </button>
...
function renderCollectionPreviewSection(items = [], total = 0, { loading = false, mode = "collection" } = {}) {
  ...
  return `
    <section class="TanadosUIwl-preview-section">
      <div class="TanadosUIwl-preview-collection-head">
        <h4 class="TanadosUIwl-preview-section-title">${escapeHtml(getContainerPreviewSectionTitle(mode))}</h4>
        ${meta ? `<div class="TanadosUIwl-preview-collection-count">${escapeHtml(meta)}</div>` : ""}
      </div>
      ${renderCollectionPreviewCards(visible, { mode })}
      ${loading ? `<div class="TanadosUIwl-preview-collection-note">${escapeHtml(getContainerPreviewLoadingText(mode))}</div>` : ""}
    </section>
  `;
}
...
function renderStatsCard(label, value) {
  return `
    <article class="TanadosUIwl-stat-card">
      <div class="TanadosUIwl-stat-label">${escapeHtml(label)}</div>
      <div class="TanadosUIwl-stat-value">${escapeHtml(formatCount(value))}</div>
    </article>
  `;
}
...
function renderWatchlistHistoryTypeCard(typeSummary) {
  return `
    <article class="TanadosUIwl-stats-type-card">
      <div class="TanadosUIwl-stats-type-name">${escapeHtml(typeSummary.label)}</div>
      <div class="TanadosUIwl-stats-type-total">
        <span>${escapeHtml(L("watchlistStatsTracked", "Toplam kayıt"))}</span>
        <strong>${escapeHtml(formatCount(typeSummary.totalEverAdded))}</strong>
      </div>
      <div class="TanadosUIwl-stats-type-row">
...
function renderWatchlistHistorySection(model) {
  ...
  return `
    <section class="TanadosUIwl-stats-shell TanadosUIwl-stats-shell-history">
      <article class="TanadosUIwl-stats-hero">
        <div class="TanadosUIwl-stats-hero-content">
          <div class="TanadosUIwl-stats-kicker">${escapeHtml(L("watchlistHistoryTitle", "Watchlist Geçmişi"))}</div>
          <h3 class="TanadosUIwl-stats-user">${escapeHtml(summary.userName)}</h3>
          <p class="TanadosUIwl-stats-copy">${escapeHtml(L("watchlistHistorySubtitle", "Aktif listen, kaldırılmış içerikler ve geçmiş toplamlar burada birlikte tutulur."))}</p>
          <div class="TanadosUIwl-stats-total">
            <span class="TanadosUIwl-stats-total-label">${escapeHtml(L("watchlistStatsTracked", "Toplam kayıt"))}</span>
            <strong class="TanadosUIwl-stats-total-value">${escapeHtml(formatCount(summary.totalEverAdded))}</strong>
          </div>
        </div>
        <div class="TanadosUIwl-stats-hero-art" aria-hidden="true">
          <div class="TanadosUIwl-stats-hero-art-badge">
            ${renderWatchlistIconSvg("TanadosUIwl-stats-hero-art-icon")}
          </div>
          <div class="TanadosUIwl-stats-hero-art-bars">
            <span></span>
            <span></span>
            <span></span>
          </div>
      ...
      <div class="TanadosUIwl-stats-cards">
      ...
      <section class="TanadosUIwl-stats-breakdown">
        <div class="TanadosUIwl-stats-breakdown-head">
          <h3 class="TanadosUIwl-stats-breakdown-title">${escapeHtml(L("watchlistStatsByType", "Türlere göre dağılım"))}</h3>
        </div>
        <div class="TanadosUIwl-stats-type-grid">
...
function renderGeneralStatsMediaCard(section) {
  ...
  const lastItemMarkup = lastItem
    ? `
      <div class="TanadosUIwl-general-last-title">${escapeHtml(lastItem.title)}</div>
      ${lastItem.subtitle ? `<div class="TanadosUIwl-general-last-subtitle">${escapeHtml(lastItem.subtitle)}</div>` : ""}
      <div class="TanadosUIwl-general-last-meta">${escapeHtml(lastItem.playedAt ? formatDate(lastItem.playedAt) : L("watchlistGeneralEmptyLast", "Henüz oynatma yok"))}</div>
    `
    : `<div class="TanadosUIwl-general-last-empty">${escapeHtml(L("watchlistGeneralEmptyLast", "Henüz oynatma yok"))}</div>`;

  return `
    <article class="TanadosUIwl-general-card" data-media-key="${escapeHtml(section?.key || "")}">
      <div class="TanadosUIwl-general-card-head">
        <h4 class="TanadosUIwl-general-card-title">${escapeHtml(section?.label || "")}</h4>
        <span class="TanadosUIwl-general-card-badge" aria-hidden="true">
          ${renderWatchlistIconSvg("TanadosUIwl-general-card-badge-icon")}
        </span>
      </div>
      <div class="TanadosUIwl-general-card-stats">
        <div class="TanadosUIwl-general-card-stat">
      ...
      <div class="TanadosUIwl-general-last">
        <div class="TanadosUIwl-general-last-label">${escapeHtml(L("watchlistGeneralLastActivity", "Son hareket"))}</div>
        ${lastItemMarkup}
      </div>
    </article>
  `;
...
function renderGeneralTopRepeated(topRepeated = []) {
  if (!Array.isArray(topRepeated) || !topRepeated.length) {
    return `<div class="TanadosUIwl-empty">${escapeHtml(L("watchlistGeneralTopReplayEmpty", "Tekrar oynatma verisi henüz yok."))}</div>`;
  }

  return `
    <div class="TanadosUIwl-general-repeat-list">
      ${topRepeated.map((item) => `
        <article class="TanadosUIwl-general-repeat-item">
          <div class="TanadosUIwl-general-repeat-main">
            <div class="TanadosUIwl-general-repeat-title">${escapeHtml(item.title)}</div>
            <div class="TanadosUIwl-general-repeat-subtitle">${escapeHtml([item.label, item.subtitle].filter(Boolean).join(" • "))}</div>
          </div>
          <div class="TanadosUIwl-general-repeat-count">${escapeHtml(formatCount(item.playCount))}</div>
        </article>
      `).join("")}
    </div>
  `;
...
function renderWatchlistGeneralSection() {
  ...
  return `
    <section class="TanadosUIwl-stats-shell TanadosUIwl-stats-shell-general">
      <article class="TanadosUIwl-stats-hero TanadosUIwl-stats-hero-secondary">
        <div class="TanadosUIwl-stats-hero-content">
          <div class="TanadosUIwl-stats-kicker">${escapeHtml(L("watchlistGeneralTitle", "Jellyfin Geneli"))}</div>
          <h3 class="TanadosUIwl-stats-user">${escapeHtml(L("watchlistGeneralHeroTitle", "Kullanıcı Medya Özeti"))}</h3>
          <p class="TanadosUIwl-stats-copy">${escapeHtml(L("watchlistGeneralSubtitle", "Toplam içerik, son oynatmalar ve tekrar izleme alışkanlığı burada gösterilir."))}</p>
        </div>
        <div class="TanadosUIwl-stats-hero-art" aria-hidden="true">
          <div class="TanadosUIwl-stats-hero-art-badge">
            ${renderWatchlistIconSvg("TanadosUIwl-stats-hero-art-icon")}
      ...
      <section class="TanadosUIwl-stats-breakdown">
        <div class="TanadosUIwl-stats-breakdown-head">
          <h3 class="TanadosUIwl-stats-breakdown-title">${escapeHtml(L("watchlistGeneralTitle", "Jellyfin Geneli"))}</h3>
        </div>
        ${loading
          ? `<div class="TanadosUIwl-loading">${escapeHtml(L("watchlistGeneralLoading", "Genel istatistikler yükleniyor..."))}</div>`
          : (media.length
            ? `
              <div class="TanadosUIwl-general-grid">
                ${media.map(renderGeneralStatsMediaCard).join("")}
              </div>
            `
            : `<div class="TanadosUIwl-empty">${escapeHtml(L("watchlistGeneralNoData", "Genel istatistik verisi bulunamadı."))}</div>`)}
      </section>

      <section class="TanadosUIwl-stats-breakdown">
        <div class="TanadosUIwl-stats-breakdown-head">
          <h3 class="TanadosUIwl-stats-breakdown-title">${escapeHtml(L("watchlistGeneralTopReplay", "En çok tekrar izlenen / dinlenenler"))}</h3>
        </div>
        ${loading
          ? `<div class="TanadosUIwl-loading">${escapeHtml(L("watchlistGeneralLoading", "Genel istatistikler yükleniyor..."))}</div>`
...
function renderWatchlistStatsPanel(model) {
  return `
    <section class="TanadosUIwl-stats-page">
...
function renderPreviewEmptyState() {
  return `
    <div class="TanadosUIwl-preview-empty">
      <div class="TanadosUIwl-preview-empty-copy">
...
function renderPreviewStats(stats = []) {
  ...
  return `
    <div class="TanadosUIwl-preview-stats">
      ${stats.map((stat) => `
        <div class="TanadosUIwl-preview-stat">
          <div class="TanadosUIwl-preview-stat-label">${escapeHtml(stat.label)}</div>
          <div class="TanadosUIwl-preview-stat-value">${escapeHtml(stat.value)}</div>
        </div>
      `).join("")}
    </div>
  `;
...
function renderPreviewFieldSection(title, fields = []) {
  ...
  return `
    <section class="TanadosUIwl-preview-section">
      <h4 class="TanadosUIwl-preview-section-title">${escapeHtml(title)}</h4>
      <div class="TanadosUIwl-preview-field-list">
        ${visible.map((field) => `
          <div class="TanadosUIwl-preview-field">
            <div class="TanadosUIwl-preview-field-label">${escapeHtml(field.label)}</div>
            <div class="TanadosUIwl-preview-field-value">${escapeHtml(field.value)}</div>
          </div>
        `).join("")}
      </div>
    </section>
...
function renderPreviewListSection(title, items = []) {
  ...
  return `
    <section class="TanadosUIwl-preview-section">
      <h4 class="TanadosUIwl-preview-section-title">${escapeHtml(title)}</h4>
      <ul class="TanadosUIwl-preview-list">
...
function renderPreviewTagSection(title, items = []) {
  ...
  return `
    <section class="TanadosUIwl-preview-section">
      <h4 class="TanadosUIwl-preview-section-title">${escapeHtml(title)}</h4>
      <div class="TanadosUIwl-preview-tags">
        ${visible.map((item) => `<span class="TanadosUIwl-preview-tag">${escapeHtml(item)}</span>`).join("")}
      </div>
    </section>
  `;
}
    ...
    <section class="TanadosUIwl-preview-section">
      ...
      <div class="TanadosUIwl-preview-tags">
        ${visible.map((studio) => {
          ...
          if (!studioId) {
            return `<span class="TanadosUIwl-preview-tag">${escapeHtml(name)}</span>`;
          }

          return `
            <button
              type="button"
              class="TanadosUIwl-preview-tag TanadosUIwl-preview-tag-button"
              data-TanadosUIwl-studio-id="${escapeHtml(studioId)}"
              data-TanadosUIwl-studio-name="${escapeHtml(name)}"
...
function renderPreviewPanel(view, details, { loading = false, collectionLoading = false } = {}) {
  ...
  return `
    <div class="TanadosUIwl-preview-shell">
      <div class="TanadosUIwl-preview-hero">
        ${backdropUrl ? `<img class="TanadosUIwl-preview-backdrop" src="${escapeHtml(backdropUrl)}" alt="" loading="eager" fetchpriority="high" decoding="async">` : ""}
        <div class="TanadosUIwl-preview-hero-inner">
          <div class="TanadosUIwl-preview-poster">
            ${posterUrl
              ? `<img src="${escapeHtml(posterUrl)}" alt="${escapeHtml(title)}" loading="eager" fetchpriority="high" decoding="async">`
              : `<div class="TanadosUIwl-preview-poster-fallback">${escapeHtml(itemType)}</div>`}
          </div>
          <div class="TanadosUIwl-preview-head">
            <div class="TanadosUIwl-preview-kicker">${escapeHtml(itemType)}</div>
            <h3 class="TanadosUIwl-preview-title">${escapeHtml(title)}</h3>
            ${subtitleLine ? `<div class="TanadosUIwl-preview-subtitle">${escapeHtml(subtitleLine)}</div>` : ""}
            ${infoLine ? `<div class="TanadosUIwl-preview-subtitle">${escapeHtml(infoLine)}</div>` : ""}
            ${chips.length ? `<div class="TanadosUIwl-preview-chips">${chips.map((chip, index) => `<span class="TanadosUIwl-preview-chip ${index === 1 ? "accent" : ""}">${escapeHtml(chip)}</span>`).join("")}</div>` : ""}
            ${progressPercent > 0 ? `
              <div class="TanadosUIwl-preview-progress">
                <div class="TanadosUIwl-preview-progress-track">
                  <div class="TanadosUIwl-preview-progress-bar" style="width:${Math.max(0, Math.min(100, progressPercent))}%"></div>
                </div>
                <div class="TanadosUIwl-preview-progress-copy">
      ...
      <div class="TanadosUIwl-preview-body">
        ${loading ? `<div class="TanadosUIwl-preview-loading">${escapeHtml(L("watchlistPreviewLoading", "Detaylar yükleniyor"))}</div>` : ""}
        ${note ? `<p class="TanadosUIwl-preview-note"><strong>${escapeHtml(L("watchlistShareNote", "Not"))}:</strong> ${escapeHtml(note)}</p>` : ""}
        <p class="TanadosUIwl-preview-overview">${escapeHtml(overview)}</p>
...
function setPreviewActiveCard(root, itemId) {
  ...
  const nextCard = root.querySelector(`[data-TanadosUIwl-item="${escapeAttrSelector(id)}"]`);
...
function applyPreviewPanelMarkup(panel, markup, { preserveMedia = false } = {}) {
  ...
  const currentShell = panel.querySelector(".TanadosUIwl-preview-shell");
  if (!currentShell) {
    panel.innerHTML = markup;
    panel.__previewMarkup = markup;
    return;
  ...
  const nextShell = template.content.querySelector(".TanadosUIwl-preview-shell");
  if (!nextShell) {
    panel.innerHTML = markup;
    panel.__previewMarkup = markup;
    return;
  ...
  const currentBackdrop = currentShell.querySelector(".TanadosUIwl-preview-backdrop");
  const nextBackdrop = nextShell.querySelector(".TanadosUIwl-preview-backdrop");
  ...
  const currentPoster = currentShell.querySelector(".TanadosUIwl-preview-poster img");
  const nextPoster = nextShell.querySelector(".TanadosUIwl-preview-poster img");
...
async function updatePreviewPanel(root, itemId) {
  const panel = root?.querySelector?.(".TanadosUIwl-preview");
  ...
  const freshPanel = root?.querySelector?.(".TanadosUIwl-preview");
...
function renderShareSummary(outgoingShares = []) {
  ...
  return `<div class="TanadosUIwl-item-sharemeta">${escapeHtml(L("watchlistSharedWith", "Paylaşıldı"))}: ${escapeHtml(names.join(", "))}</div>`;
}
...
function renderItemCard(view) {
  ...
  const poster = item.posterUrl
    ...
    : `<div class="TanadosUIwl-item-poster-fallback">${escapeHtml(typeLabel)}</div>`;
  ...
  const noteHtml = view.kind === "shared" && view.note
    ? `<div class="TanadosUIwl-item-sharemeta"><strong>${escapeHtml(L("watchlistShareNote", "Not"))}:</strong> ${escapeHtml(view.note)}</div>`
    : "";

  const shareMeta = view.kind === "shared"
    ? `<div class="TanadosUIwl-item-sharemeta">${escapeHtml(L("watchlistSharedBy", "Paylaşan"))}: ${escapeHtml(view.ownerUserName)}${view.sharedAtUtc ? ` • ${escapeHtml(formatDate(view.sharedAtUtc))}` : ""}</div>`
    : renderShareSummary(view.outgoingShares);

  const secondaryAction = view.kind === "own"
    ? `<button class="TanadosUIwl-btn" data-TanadosUIwl-share="${escapeHtml(view.itemId)}">${escapeHtml(L("watchlistShareAction", "Paylaş"))}</button>`
    : "";

  return `
    <article class="TanadosUIwl-item ${isPlayed ? "is-played" : ""}" tabindex="0" data-TanadosUIwl-item="${escapeHtml(view.itemId)}" data-TanadosUIwl-kind="${escapeHtml(view.kind)}" data-TanadosUIwl-view-key="${escapeHtml(text(view?.key, `${view?.kind === "shared" ? "shared" : "own"}:${view?.kind === "shared" ? text(view?.shareId) : text(view?.itemId)}`))}" ${view.kind === "shared" && text(view?.shareId) ? `data-TanadosUIwl-share-id="${escapeHtml(text(view.shareId))}"` : ""}>
      <div class="TanadosUIwl-item-poster">
        ${poster}
        ${isPlayed ? renderPlayedOverlayMarkup() : ""}
      </div>
      <div class="TanadosUIwl-item-main">
        <h3 class="TanadosUIwl-item-title">${escapeHtml(item.name)}</h3>
        ${meta ? `<div class="TanadosUIwl-item-meta">${escapeHtml(meta)}</div>` : ""}
        ${extraLine ? `<div class="TanadosUIwl-item-extra">${extraLine}</div>` : ""}
        <div class="TanadosUIwl-item-overview">${escapeHtml(item.overview)}</div>
        ${tags.length ? `<div class="TanadosUIwl-item-tags">${tags.map((tag) => `<span class="TanadosUIwl-tag">${escapeHtml(tag)}</span>`).join("")}</div>` : ""}
        ${shareMeta}
        ${noteHtml}
        <div class="TanadosUIwl-item-actions">
          <button class="TanadosUIwl-btn primary" data-TanadosUIwl-play-now="${escapeHtml(view.itemId)}">${escapeHtml(playActionLabel)}</button>
          ${secondaryAction}
          <button class="TanadosUIwl-btn danger" data-TanadosUIwl-remove="${escapeHtml(view.kind === "shared" ? view.shareId : view.itemId)}" data-TanadosUIwl-remove-kind="${escapeHtml(view.kind)}">${escapeHtml(L("watchlistRemoveAction", "Kaldır"))}</button>
        </div>
      </div>
    </article>
  `;
...
function findWatchlistSectionElement(root, sectionKey) {
  ...
  const directMatch = root.querySelector?.(`[data-TanadosUIwl-section="${escapeAttrSelector(key)}"]`);
  if (directMatch) return directMatch;

  const sections = root.querySelectorAll?.(".TanadosUIwl-main .TanadosUIwl-section");
...
function renderSection(title, items, sectionKey = "") {
  ...
  if (!list.length) {
    return `
      <section class="TanadosUIwl-section" ${sectionKey ? `data-TanadosUIwl-section="${escapeHtml(sectionKey)}"` : ""}>
        <div class="TanadosUIwl-section-head">
          <h3 class="TanadosUIwl-section-title">${escapeHtml(sectionTitle)}</h3>
        </div>
        <div class="TanadosUIwl-empty">${escapeHtml(L("watchlistEmptySection", "Burada henüz öğe yok."))}</div>
      </section>
    `;
  }
  ...
  return `
    <section class="TanadosUIwl-section" ${sectionKey ? `data-TanadosUIwl-section="${escapeHtml(sectionKey)}"` : ""}>
      ...
      <div class="TanadosUIwl-grid" ${sectionKey ? `data-TanadosUIwl-section-grid="${escapeHtml(sectionKey)}"` : ""}>${initialItems.map(renderItemCard).join("")}</div>
      ${showLoader ? `<div class="TanadosUIwl-loading" data-TanadosUIwl-section-loading="${escapeHtml(sectionKey)}">${escapeHtml(L("loading", "Yükleniyor..."))}</div>` : ""}
    </section>
  `;
}
...
function syncDeferredWatchlistFocus(root) {
  ...
  const focusCard = root.querySelector?.(`[data-TanadosUIwl-item="${escapeAttrSelector(focusItemId)}"]`);
  if (!focusCard) return;
...
function scheduleWatchlistSectionRender(root, sectionKey, items = [], startIndex = 0, renderToken = 0) {
  ...
  const run = () => {
    ...
    const grid = root.querySelector(`[data-TanadosUIwl-section-grid="${escapeAttrSelector(sectionKey)}"]`);
    if (!grid) return;

    const nextItems = list.slice(startIndex, startIndex + WATCHLIST_PROGRESSIVE_BATCH_SIZE);
    if (!nextItems.length) {
      root.querySelector(`[data-TanadosUIwl-section-loading="${escapeAttrSelector(sectionKey)}"]`)?.remove();
      return;
    }
...
function scheduleProgressiveWatchlistSections(root, model, activeTab) {
  ...
  if (ownStart < ownItems.length) {
    ...
    root.querySelector('[data-TanadosUIwl-section-loading="own"]')?.remove();
  }

  if (sharedStart < sharedItems.length) {
    scheduleWatchlistSectionRender(root, "shared", sharedItems, sharedStart, renderToken);
  } else {
    root.querySelector('[data-TanadosUIwl-section-loading="shared"]')?.remove();
  }
}
...
function renderModalShell(model, activeTab) {
  ...
  return `
    <div class="TanadosUIwl-backdrop">
      <div class="TanadosUIwl-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(tabTitle)}">
        <div class="TanadosUIwl-header">
          <div>
            <h2 class="TanadosUIwl-title">${escapeHtml(L("watchlistOpen", "İzleme Listesi"))}</h2>
            <p class="TanadosUIwl-subtitle">${escapeHtml(L("watchlistModalSubtitle", "Öğeler cihazdan bağımsız sunucu tarafında tutulur. İstersen diğer kullanıcılarla not ekleyerek paylaşabilirsin."))}</p>
          </div>
          <div class="TanadosUIwl-header-actions">
            <button class="TanadosUIwl-close" data-TanadosUIwl-close="1" aria-label="${escapeHtml(L("closeButton", "Kapat"))}">✕</button>
          </div>
        </div>

        <div class="TanadosUIwl-tabs">
          ${WATCHLIST_TABS.map((tab) => {
            return `<button class="TanadosUIwl-tab ${currentTab === tab.key ? "active" : ""}" data-TanadosUIwl-tab="${escapeHtml(tab.key)}">${escapeHtml(getWatchlistTabButtonText(model, tab.key))}</button>`;
          }).join("")}
        </div>

        <div class="TanadosUIwl-body">
          <div class="TanadosUIwl-layout${layoutClass}">
            <div class="TanadosUIwl-main">
              ${renderCurrentWatchlistTabSections(model, currentTab)}
            </div>
            <aside class="TanadosUIwl-preview" aria-live="polite">
...
function getWatchlistCardViewKey(card) {
  ...
  const explicitKey = text(card.getAttribute?.("data-TanadosUIwl-view-key"));
  if (explicitKey) return explicitKey;

  const kind = text(card.getAttribute?.("data-TanadosUIwl-kind"));
  const shareId = text(card.getAttribute?.("data-TanadosUIwl-share-id"));
  const itemId = text(card.getAttribute?.("data-TanadosUIwl-item"));
...
function updateWatchlistTabButtons(root, model, activeTab) {
  ...
  const currentTab = normalizeWatchlistTabKey(activeTab);
  root.querySelectorAll("[data-TanadosUIwl-tab]").forEach((button) => {
    const tabKey = normalizeWatchlistTabKey(button.getAttribute("data-TanadosUIwl-tab"));
...
function renderCurrentWatchlistTabContent(root, model, { preserveScroll = false } = {}) {
  const main = root?.querySelector?.(".TanadosUIwl-main");
...
function updateWatchlistSectionAfterRemoval(root, sectionKey, title, items, change = {}) {
  ...
  const grid = section.querySelector(`[data-TanadosUIwl-section-grid="${escapeAttrSelector(sectionKey)}"]`);
  const titleEl = section.querySelector(".TanadosUIwl-section-title");
  if (!grid || !titleEl) {
    section.outerHTML = nextMarkup;
    const initialCount = getWatchlistInitialRenderCount(list);
  ...
  const visibleCountBefore = grid.querySelectorAll(".TanadosUIwl-item").length;

  Array.from(grid.querySelectorAll(".TanadosUIwl-item")).forEach((card) => {
    const viewKey = getWatchlistCardViewKey(card);
    const cardItemId = text(card.getAttribute("data-TanadosUIwl-item"));
    const cardShareId = text(card.getAttribute("data-TanadosUIwl-share-id"));
    const shouldRemove =
  ...
  const renderedKeys = new Set(
    Array.from(grid.querySelectorAll(".TanadosUIwl-item"))
  ...
  const loadingSelector = `[data-TanadosUIwl-section-loading="${escapeAttrSelector(sectionKey)}"]`;
  const existingLoader = section.querySelector(loadingSelector);
  if (desiredRenderedCount < list.length) {
    if (!existingLoader) {
      const loader = document.createElement("div");
      loader.className = "TanadosUIwl-loading";
      loader.setAttribute("data-TanadosUIwl-section-loading", sectionKey);
...
async function renderWatchlistModal(root, state = {}) {
  ...
  try {
    ...
    if (hotModel) {
      ...
      root.innerHTML = `
        <div class="TanadosUIwl-backdrop">
          <div class="TanadosUIwl-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(L("watchlistOpen", "İzleme Listesi"))}">
            <div class="TanadosUIwl-header">
              <div>
                <h2 class="TanadosUIwl-title">${escapeHtml(L("watchlistOpen", "İzleme Listesi"))}</h2>
                <p class="TanadosUIwl-subtitle">${escapeHtml(L("loading", "Yükleniyor..."))}</p>
              </div>
              <div class="TanadosUIwl-header-actions">
                <button class="TanadosUIwl-close" data-TanadosUIwl-close="1" aria-label="${escapeHtml(L("closeButton", "Kapat"))}">✕</button>
              </div>
            ...
            <div class="TanadosUIwl-body">
              <div class="TanadosUIwl-loading">${escapeHtml(L("loading", "Yükleniyor..."))}</div>
            </div>
          </div>
        </div>
      `;
      ...
      <div class="TanadosUIwl-backdrop">
        <div class="TanadosUIwl-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(L("watchlistOpen", "İzleme Listesi"))}">
          <div class="TanadosUIwl-header">
            <div>
              <h2 class="TanadosUIwl-title">${escapeHtml(L("watchlistOpen", "İzleme Listesi"))}</h2>
              <p class="TanadosUIwl-subtitle">${escapeHtml(L("watchlistLoadError", "İzleme listesi yüklenemedi."))}</p>
            </div>
            <div class="TanadosUIwl-header-actions">
              <button class="TanadosUIwl-close" data-TanadosUIwl-close="1" aria-label="${escapeHtml(L("closeButton", "Kapat"))}">✕</button>
            </div>
          ...
          <div class="TanadosUIwl-body">
            <div class="TanadosUIwl-error">${escapeHtml(error?.message || L("watchlistLoadError", "İzleme listesi yüklenemedi."))}</div>
          </div>
        </div>
      </div>
    `;
...
function bindModalInteractions(root) {
  ...
  root.addEventListener("mouseover", (event) => {
    const preview = event.target?.closest?.(".TanadosUIwl-preview");
    ...
    const card = event.target?.closest?.(".TanadosUIwl-item");
  ...
  root.addEventListener("click", async (event) => {
    if (event.target?.closest?.("[data-TanadosUIwl-close='1']")) return;

    const tabButton = event.target?.closest?.("[data-TanadosUIwl-tab]");
    if (tabButton && root.contains(tabButton)) {
      const nextTab = normalizeWatchlistTabKey(tabButton.getAttribute("data-TanadosUIwl-tab"));
    ...
    const studioButton = event.target?.closest?.("[data-TanadosUIwl-studio-id]");
    if (studioButton && root.contains(studioButton)) {
      ...
      const studioId = text(studioButton.getAttribute("data-TanadosUIwl-studio-id"));
      const studioName = text(studioButton.getAttribute("data-TanadosUIwl-studio-name"));
      if (!studioId) {
    ...
    const previewPlayButton = event.target?.closest?.("[data-TanadosUIwl-preview-play]");
    if (previewPlayButton && root.contains(previewPlayButton)) {
      event.preventDefault();
      event.stopPropagation();

      const itemId = text(previewPlayButton.getAttribute("data-TanadosUIwl-preview-play"));
      if (!itemId) return;
      await startWatchlistPlayback(previewPlayButton, itemId);
      return;
    }

    const playNowButton = event.target?.closest?.("[data-TanadosUIwl-play-now]");
    if (playNowButton && root.contains(playNowButton)) {
      const itemId = text(playNowButton.getAttribute("data-TanadosUIwl-play-now"));
      if (!itemId) return;
      await startWatchlistPlayback(playNowButton, itemId);
      return;
    }

    const removeButton = event.target?.closest?.("[data-TanadosUIwl-remove]");
    if (removeButton && root.contains(removeButton)) {
      const removeKind = text(removeButton.getAttribute("data-TanadosUIwl-remove-kind"));
      const targetId = text(removeButton.getAttribute("data-TanadosUIwl-remove"));
      if (!targetId) return;
    ...
    const shareButton = event.target?.closest?.("[data-TanadosUIwl-share]");
    if (shareButton && root.contains(shareButton)) {
      const itemId = text(shareButton.getAttribute("data-TanadosUIwl-share"));
      if (!itemId) return;
    ...
    const card = event.target?.closest?.(".TanadosUIwl-item");
    if (!card || !root.contains(card)) return;
    if (event.target?.closest?.(".TanadosUIwl-btn")) return;
...
async function openShareOverlay(root, itemId) {
  ...
  overlay.className = "TanadosUIwl-share-overlay";
  overlay.innerHTML = `
    <div class="TanadosUIwl-share-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(shareTitle)}">
      <h3 class="TanadosUIwl-share-title">${escapeHtml(shareTitle)}</h3>
      <p class="TanadosUIwl-share-help">${escapeHtml(L("watchlistShareSubtitle", "Birden fazla kullanıcı seçebilir ve paylaşım sırasında kısa bir not ekleyebilirsin."))}</p>
      <div class="TanadosUIwl-share-list">
        ${users.length
          ? users.map((user) => `
            <label class="TanadosUIwl-share-user">
              <input type="checkbox" value="${escapeHtml(user.id)}">
              <span>${escapeHtml(user.name)}</span>
            </label>
          `).join("")
          : `<div class="TanadosUIwl-empty">${escapeHtml(L("watchlistNoUsers", "Paylaşılacak kullanıcı bulunamadı."))}</div>`
        }
      </div>
      <label class="TanadosUIwl-share-note-label" for="TanadosUIwl-share-note">${escapeHtml(L("watchlistShareNoteLabel", "Paylaşım notu"))}</label>
      <textarea id="TanadosUIwl-share-note" class="TanadosUIwl-share-note" placeholder="${escapeHtml(L("watchlistShareNotePlaceholder", "İstersen kısa bir not bırakabilirsin."))}"></textarea>
      <div class="TanadosUIwl-share-footer">
        <button class="TanadosUIwl-share-cancel" type="button">${escapeHtml(L("cancel", "İptal"))}</button>
        <button class="TanadosUIwl-share-submit" type="button">${escapeHtml(L("watchlistShareAction", "Paylaş"))}</button>
      </div>
    </div>
  `;
  ...
  overlay.querySelector(".TanadosUIwl-share-cancel")?.addEventListener("click", closeOverlay);

  overlay.querySelector(".TanadosUIwl-share-submit")?.addEventListener("click", async (event) => {
    const submitButton = event.currentTarget;
    const selectedIds = [...overlay.querySelectorAll(".TanadosUIwl-share-user input:checked")]
      .map((input) => text(input.value))
      .filter(Boolean);
    const note = text(overlay.querySelector(".TanadosUIwl-share-note")?.value);
...
try {
  window.__TanadosUIOpenWatchlistModal = openWatchlistModal;
} catch {}
