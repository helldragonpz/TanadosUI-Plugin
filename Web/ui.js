(function () {
  ...
  const webSettingsModuleUrl = `${window.location.origin}${jfRoot}/Plugins/TanadosUI/assets/WebSettingsJs`;
  const sliderSettingsCssUrl = `${window.location.origin}${jfRoot}/slider/src/settings.css`;
  const TAB_STORAGE_KEY = "TanadosUI-config-active-tab";
  const TanadosUI_SUBTAB_STORAGE_KEY = "TanadosUI-TanadosUI-requested-subtab";

  const api = (p) => `${jfRoot}/Plugins/TanadosUI/${p}`;
  ...
  function ensureStylesheet(key, href) {
    let link = document.querySelector(`link[data-TanadosUI-config-css="${key}"]`);
    if (!link) {
      link = document.createElement("link");
      link.rel = "stylesheet";
      link.setAttribute("data-TanadosUI-config-css", key);
  ...
  function renderTanadosUISettingsPlaceholder(view, text, tone = "") {
    const host = view.querySelector("#TanadosUISettingsHost");
    if (!host) return;

    const placeholder = document.createElement("div");
    placeholder.id = "TanadosUISettingsPlaceholder";
    placeholder.className = `jms-empty ${tone ? `jms-empty--${tone}` : ""}`.trim();
    placeholder.textContent = text;
    host.replaceChildren(placeholder);
  }

  function consumeRequestedTanadosUISettingsTab() {
    let value = "";
    try {
      value = sessionStorage.getItem(TanadosUI_SUBTAB_STORAGE_KEY) || "";
      if (value) sessionStorage.removeItem(TanadosUI_SUBTAB_STORAGE_KEY);
    } catch {}
    return String(value || "").trim() || "TanadosUI";
  }

  async function ensureTanadosUISettings(view, { force = false } = {}) {
    const host = view.querySelector("#TanadosUISettingsHost");
    const reloadBtn = view.querySelector("#reloadTanadosUISettingsBtn");
    if (!host) return null;

    const requestedInnerTab = consumeRequestedTanadosUISettingsTab();

    if (!force && host.__jmsTanadosUIReady && host.querySelector("#settings-modal")) {
      const existingApi = host.__jmsTanadosUIApi || host.__jmsTanadosUISettingsApi || null;
      existingApi?.open?.(requestedInnerTab);
      return host.querySelector("#settings-modal");
    }

    if (host.__jmsTanadosUIPromise) {
      return host.__jmsTanadosUIPromise;
    }
    ...
    host.__jmsTanadosUIPromise = (async () => {
      ensureStylesheet("TanadosUI-settings", sliderSettingsCssUrl);

      const settingsModule = await import(webSettingsModuleUrl);
      const settingsApi = typeof settingsModule?.mountTanadosUISettingsPage === "function"
        ? await settingsModule.mountTanadosUISettingsPage(host, {
      ...
      host.__jmsTanadosUIReady = true;
      view.__TanadosUISettingsLoaded = true;
      return modal;
    })()
      .catch((error) => {
      ...
      .finally(() => {
        host.__jmsTanadosUIPromise = null;
  ...
  function initTabs(view) {
    ...
    view.querySelectorAll(".jms-tab").forEach((tab) => {
      tab.addEventListener("click", () => activateTab(view, tab.dataset.tab || "TanadosUI"));
    });

    let active = "TanadosUI";
    try {
      const stored = localStorage.getItem(TAB_STORAGE_KEY);
      if (stored && ["TanadosUI", "TanadosUI-settings", "status", "snippet"].includes(stored)) {
        active = stored;
      }
    } catch {}
  ...
  async function initView(view) {
    ...
    view.querySelector("#publishGlobalBtn")?.addEventListener("click", async () => {
      try {
        ...
        await fetch(`${jfRoot}/Plugins/TanadosUI/UserSettings`, { cache: "no-store" }).catch(() => null);
    ...
    view.querySelector("#reloadTanadosUISettingsBtn")?.addEventListener("click", async () => {
      try {
        await ensureTanadosUISettings(view, { force: true });
      } catch (e) {
        showMessage(view, e.message || String(e), "err");
      }
    });
  ...
  async function refreshLanguageIfNeeded() {
    const view = document.getElementById("TanadosUIConfigPage");
    if (!view) return;
    await loadLanguagePack();
    applyTranslations(view);
  }
  ...
  function handlePageEvents(e) {
    ...
    if (view && (view.id === "TanadosUIConfigPage" || view.querySelector?.("#TanadosUIConfigPage"))) {
      const page = view.id === "TanadosUIConfigPage" ? view : view.querySelector("#TanadosUIConfigPage");
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const existingView = document.getElementById("TanadosUIConfigPage");
    if (existingView) setTimeout(() => initView(existingView), 50);
  });

  window.addEventListener("TanadosUI:plugin-config-open-request", (event) => {
    const detail = event?.detail || {};
    if (detail.pluginTab === "TanadosUI-settings") {
      try {
        localStorage.setItem(TAB_STORAGE_KEY, "TanadosUI-settings");
      } catch {}
      try {
        sessionStorage.setItem(TanadosUI_SUBTAB_STORAGE_KEY, String(detail.settingsTab || "TanadosUI"));
      } catch {}
    }

    const existingView = document.getElementById("TanadosUIConfigPage");
    if (existingView) {
      activateTab(existingView, detail.pluginTab || "TanadosUI");
    }
  });

  const immediateCheck = document.getElementById("TanadosUIConfigPage");
